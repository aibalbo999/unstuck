...
def test_agent_prompt_token_budget_reserves_response_and_safety_margin(monkeypatch):
    ...
    monkeypatch.setattr(generation_config, "estimate_agent_input_tokens", lambda *_: 50)

    assert prompt_budget.get_agent_prompt_token_budget(4) == 800
...
def test_enforce_prompt_token_budget_preserves_source_and_tail_for_admission():
    prompt = "\n".join(
        [
            "開頭原始資料 " * 120,
            "中段新聞與 RAG " * 1400,
    ...
    assert trimmed == prompt
    assert "Prompt budget guard" not in trimmed
    assert "尾端輸出規則必須保留" in trimmed
    assert trimmed.count("中段新聞與 RAG") == 1400
