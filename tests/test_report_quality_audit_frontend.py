...
const payload = {
  ...
  report_quality_audit: {
    current_quality_summary: {
      ...
      items_returned: 1,
      items_truncated: false,
      items: [{ ticker: '2330.TW', pipeline_id: 'v4', filename: '2330_v4.html', report_conformance_status: 'warning', content_credibility_status: 'warning', evidence_exit_gate_verdict: 'caution' }]
    }
  }
};
const board = window.StockAgentWatchlistPanelHelpers.watchlistDailyBoard([], payload, value => String(value ?? ''));
...
def test_watchlist_board_does_not_trust_current_quality_items_limit():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    current_quality_helper_path = STATIC_DIR / "watchlist_current_quality_helpers.js"
    scope_path = STATIC_DIR / "report_quality_queue_scope_helpers.js"
    script = """
...
const payload = {
  ...
  report_quality_audit: {
    current_quality_summary: {
      ...
      items_returned: 2,
      items_truncated: false,
      items: [{ ticker: '2330.TW', pipeline_id: 'v4', filename: '2330_v4.html', report_conformance_status: 'warning', content_credibility_status: 'warning', evidence_exit_gate_verdict: 'caution' }, { ticker: '2317.TW', pipeline_id: 'v4', filename: '2317_v4.html', report_conformance_status: 'warning', content_credibility_status: 'warning', evidence_exit_gate_verdict: 'caution' }]
    }
  }
};
const board = window.StockAgentWatchlistPanelHelpers.watchlistDailyBoard([], payload, value => String(value ?? ''));
...
""".replace("__SCOPE_PATH__", json.dumps(str(scope_path))).replace("__CURRENT_QUALITY_HELPER_PATH__", json.dumps(str(current_quality_helper_path))).replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    ...
    assert "目前品質待查看（顯示 2/2；範圍資料需確認）" in payload["board"]
    assert "目前品質待查看（2）" not in payload["board"]
...
def test_watchlist_board_rejects_passed_current_quality_target():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    current_quality_helper_path = STATIC_DIR / "watchlist_current_quality_helpers.js"
    script = """
global.window = {};
...
const currentQuality = {
  ...
  items_total: 1,
  items_returned: 1,
  report_conformance_by_status: { passed: 1, warning: 1, blocked: 0, unknown: 0 },
  ...
  items: [{ ticker: 'PASS', pipeline_id: 'v1', filename: 'pass.html', report_conformance_status: 'passed', content_credibility_status: 'passed', evidence_exit_gate_verdict: 'approved' }]
};
const board = window.StockAgentWatchlistPanelHelpers.watchlistDailyBoard([], {
  decision_queue: { summary: { total_actionable: 0 }, items: [{ type: 'monitor' }] },
  report_quality_audit: { current_quality_summary: currentQuality }
}, value => String(value ?? ''));
process.stdout.write(JSON.stringify({ board }));
""".replace("__CURRENT_QUALITY_HELPER_PATH__", json.dumps(str(current_quality_helper_path))).replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    result = subprocess.run(["node", "-e", script], check=True, capture_output=True, text=True)
    payload = json.loads(result.stdout)

    assert "目前品質待查看" not in payload["board"]
    assert 'data-quality-history-query="pass.html"' not in payload["board"]


def test_watchlist_board_rejects_current_quality_target_without_filename():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    current_quality_helper_path = STATIC_DIR / "watchlist_current_quality_helpers.js"
    script = """
global.window = {};
...
const currentQuality = {
  ...
  items: [{ ticker: 'NOFILE', pipeline_id: 'v1', report_conformance_status: 'warning', content_credibility_status: 'passed', evidence_exit_gate_verdict: 'caution' }]
};
const board = window.StockAgentWatchlistPanelHelpers.watchlistDailyBoard([], {
  decision_queue: { summary: { total_actionable: 0 }, items: [{ type: 'monitor' }] },
  report_quality_audit: { current_quality_summary: currentQuality }
...
def test_watchlist_board_does_not_render_fractional_freshness_summary():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    freshness_helper_path = STATIC_DIR / "watchlist_freshness_helpers.js"
    script = """
global.window = {};
...
""".replace("__FRESHNESS_HELPER_PATH__", json.dumps(str(freshness_helper_path))).replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    ...
    assert "分析新鮮度：" not in payload["board"]


def test_watchlist_board_does_not_render_fractional_current_quality_summary():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    current_quality_helper_path = STATIC_DIR / "watchlist_current_quality_helpers.js"
    script = """
global.window = {};
...
def test_watchlist_board_rejects_current_quality_non_passed_above_audited_scope():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    current_quality_helper_path = STATIC_DIR / "watchlist_current_quality_helpers.js"
    script = """
global.window = {};
...
""".replace("__CURRENT_QUALITY_HELPER_PATH__", json.dumps(str(current_quality_helper_path))).replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    ...
    assert "目前品質：" not in payload["board"]
    assert "非通過 2" not in payload["board"]


def test_watchlist_board_rejects_unknown_current_quality_status_key():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    current_quality_helper_path = STATIC_DIR / "watchlist_current_quality_helpers.js"
    script = """
global.window = {};
...
const board = window.StockAgentWatchlistPanelHelpers.watchlistDailyBoard([], {
  ...
  report_quality_audit: {
    ...
    repair_sample_overlap: {
      status: 'complete',
      audit_gap_reports: 2.5,
      audit_gap_reports_in_repair_sample: 1.5,
      audit_gap_reports_outside_repair_sample: 1,
...
""".replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    ...
    assert "品質缺口與 repair sample：" not in payload["board"]
    assert "2.5" not in payload["board"]
    assert "1.5" not in payload["board"]


def test_watchlist_board_does_not_render_inconsistent_repair_sample_overlap_counts():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    script = """
global.window = {};
require(__HELPER_PATH__);
const board = window.StockAgentWatchlistPanelHelpers.watchlistDailyBoard([], {
  ...
  report_quality_audit: {
    ...
    repair_sample_overlap: {
      status: 'complete',
      audit_gap_reports: 2,
      audit_gap_reports_in_repair_sample: 3,
      audit_gap_reports_outside_repair_sample: 0,
      audit_gap_items_returned: 2
    },
    items: []
  }
...
""".replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    ...
    assert "品質缺口與 repair sample：" not in payload["board"]
    assert "3/2" not in payload["board"]


def test_watchlist_board_does_not_render_complete_overlap_with_missing_items():
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    script = """
global.window = {};
require(__HELPER_PATH__);
...
""".replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    ...
    assert "品質缺口與 repair sample：" not in payload["board"]
    assert "0/2" not in payload["board"]
...
const payload = {
  ...
  report_quality_audit: {
    ...
    decision_freshness_items: {
      ...
      items_limit: 1,
      items_truncated: false,
      items: [{ ticker: '3653.TW', pipeline_id: 'v3', filename: '3653_v3.html' }, { ticker: '2308.TW', pipeline_id: 'v3', filename: '2308_v3.html' }]
    }
  }
};
const board = window.StockAgentWatchlistPanelHelpers.watchlistDailyBoard([], payload, value => String(value ?? ''));
...
""".replace("__SCOPE_PATH__", json.dumps(str(scope_path))).replace("__FRESHNESS_HELPER_PATH__", json.dumps(str(freshness_helper_path))).replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    ...
    assert "待重跑報告（顯示 2/2；範圍資料需確認）" in payload["board"]
    assert "待重跑報告（2）" not in payload["board"]
...
def test_watchlist_board_surfaces_current_quality_projection_separately_with_history_navigation():
    ...
    helper_path = STATIC_DIR / "watchlist_panel_helpers.js"
    freshness_helper_path = STATIC_DIR / "watchlist_freshness_helpers.js"
    action_scope_helper_path = STATIC_DIR / "report_quality_action_scope_helpers.js"
    current_quality_helper_path = STATIC_DIR / "watchlist_current_quality_helpers.js"
    script = """
global.window = {};
require(__EVIDENCE_PATH__);
require(__FRESHNESS_HELPER_PATH__);
require(__ACTION_SCOPE_HELPER_PATH__);
require(__CURRENT_QUALITY_HELPER_PATH__);
require(__HELPER_PATH__);
const payload = {
  decision_queue: { summary: { total_actionable: 0 }, items: [{ type: 'monitor' }] },
...
""".replace("__EVIDENCE_PATH__", json.dumps(str(evidence_path))).replace("__FRESHNESS_HELPER_PATH__", json.dumps(str(freshness_helper_path))).replace("__ACTION_SCOPE_HELPER_PATH__", json.dumps(str(action_scope_helper_path))).replace("__CURRENT_QUALITY_HELPER_PATH__", json.dumps(str(current_quality_helper_path))).replace("__HELPER_PATH__", json.dumps(str(helper_path)))
    result = subprocess.run(["node", "-e", script], check=True, capture_output=True, text=True)
    payload = json.loads(result.stdout)
