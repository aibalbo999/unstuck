...
def test_shared_quality_evidence_helper_loads_before_all_consumers():
    ...
    assert "/static/history_current_quality_helpers.js?v=20260902-known-status-scope" in index_html
    assert "/static/watchlist_freshness_helpers.js?v=20260902-integer-quality-counts" in index_html
    assert "/static/watchlist_current_quality_helpers.js?v=20260902-known-status-scope" in index_html
...
process.stdout.write(window.StockAgentReportQualityEvidence.formatUnverifiableReasonFreshnessSummary({
  ...
  unknown: {}
}, { current: 1, needs_rerun: 2, unknown: 0 }, { current: 1, needs_rerun: 3, unknown: 0 }));
""".replace("__EVIDENCE_PATH__", json.dumps(str(evidence_path))).replace("__FRESHNESS_PATH__", json.dumps(str(freshness_path)))

    assert _node(script) == "證據未驗證版本：資料已更新、本文需完整重跑（衍生指標沒有 canonical 欄位 2、舊結論缺少快照路徑 1；不可驗證 claim 3；涉及 2 份報告）、本文目前版本（技術價位沒有 canonical 欄位 1；不可驗證 claim 1；涉及 1 份報告）"


def test_shared_quality_evidence_surfaces_unclassified_unverifiable_claims():
    evidence_path = STATIC_DIR / "report_quality_evidence_freshness_helpers.js"
    script = """
global.window = {};
require(__EVIDENCE_PATH__);
process.stdout.write(window.StockAgentReportQualityEvidence.formatUnverifiableReasonFreshnessSummary(
  { current: {}, needs_rerun: {}, unknown: {} },
  { current: 1, needs_rerun: 0, unknown: 0 },
  { current: 2, needs_rerun: 0, unknown: 0 }
));
""".replace("__EVIDENCE_PATH__", json.dumps(str(evidence_path)))

    assert _node(script) == "證據未驗證版本：本文目前版本（原因未記錄；不可驗證 claim 2；涉及 1 份報告）"
...
def test_shared_quality_evidence_formats_content_blocker_freshness():
    evidence_path = STATIC_DIR / "report_quality_evidence_helpers.js"
    script = """
global.window = {};
require(__EVIDENCE_PATH__);
process.stdout.write(window.StockAgentReportQualityEvidence.formatContentBlockerFreshnessSummary({
  needs_rerun: 8,
  current: 5,
  unknown: 0
}));
...
def test_shared_quality_evidence_formats_per_report_content_blocker_messages():
    evidence_path = STATIC_DIR / "report_quality_evidence_helpers.js"
    script = """
global.window = {};
require(__EVIDENCE_PATH__);
process.stdout.write(window.StockAgentReportQualityEvidence.formatQualityBlockerMessages([
  'Agent 7 輸出失敗。',
  'Agent 7 輸出失敗。',
  '偏多交易的目標價未高於目前股價。'
]));
...
def test_shared_quality_audit_scope_validates_complete_distributions():
    scope_path = STATIC_DIR / "report_quality_audit_scope_helpers.js"
    script = """
global.window = {};
require(__SCOPE_PATH__);
const valid = window.StockAgentReportQualityAuditScope.completeDistribution;
process.stdout.write(JSON.stringify({
  valid: valid({ before_refresh: 1, after_refresh: 1 }, 2, ['before_refresh', 'after_refresh']),
  zeroOmitted: valid({ before_refresh: 2 }, 2, ['before_refresh', 'after_refresh']),
  mismatch: valid({ before_refresh: 1 }, 2, ['before_refresh', 'after_refresh']),
  fractional: valid({ before_refresh: 1.5, after_refresh: 0.5 }, 2, ['before_refresh', 'after_refresh']),
  unknown: valid({ before_refresh: 2, future: 1 }, 3, ['before_refresh', 'after_refresh'])
}));
""".replace("__SCOPE_PATH__", json.dumps(str(scope_path)))
