...
def test_evidence_gate_matches_recent_trend_heading_using_snapshot_year():
    from evidence_exit_gate import evaluate_report_evidence

    result = evaluate_report_evidence(
        "* Section 1: Institutional Trading (Foreign & Investment Trust)\n"
        "  * Recent Trend (Last 10 days):\n"
        "    * Aug 20: -26.48\n"
        "    * Aug 24: -599.3 (Significant drop)",
        {
            "data": {
                "price_history": {
                    "dates": ["2026-08-29"],
...
def test_evidence_gate_maps_swing_ceiling_to_week_high():
    from evidence_exit_gate import evaluate_report_evidence

    result = evaluate_report_evidence(
        "* **波段天花板：** 73.6 TWD（52 週最高價，`market_data`）。",
        {"data": {"week_52_high": 73.6}},
        sample_ratio=1.0,
        min_sample=1,
    )
...
def test_evidence_gate_does_not_infer_swing_ceiling_without_week_marker():
    from evidence_exit_gate import evaluate_report_evidence

    result = evaluate_report_evidence(
        "* **波段天花板：** 73.6 TWD（長期結構性壓力）。",
        {"data": {"week_52_high": 73.6}},
        sample_ratio=1.0,
        min_sample=1,
    )
