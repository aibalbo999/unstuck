(function () {
    ...
    function watchlistDailyBoard(items, daily, escapeHtml) {
        ...
        const auditButtons = auditItems.map(item => {
            ...
            const targetContext = [reviewSummary, evidence.targetContext || ''].filter(Boolean).join('；'), reviewStatusHtml = reviewSummary ? `<small class="watchlist-quality-review-status">${escapeHtml(reviewSummary)}</small>` : '', evidenceContextHtml = evidence.targetContext ? `<small class="watchlist-quality-evidence-context">${escapeHtml(evidence.targetContext)}</small>` : '';
