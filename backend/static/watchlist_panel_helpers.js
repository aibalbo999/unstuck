(function () {
    ...
    function watchlistDailyBoard(items, daily, escapeHtml) {
        ...
        const historicalAuditButton = audit.selection_basis === 'latest_per_ticker_pipeline' && missingQuality > 0
            ...
            : '';
        const freshnessTargetsHtml = window.StockAgentWatchlistFreshnessHelpers?.targets?.(audit, escapeHtml) || ''; const auditControls = freshnessTargetsHtml || historicalAuditButton || auditButtons ? `<div class="watchlist-quality-audit-actions">${freshnessTargetsHtml}${historicalAuditButton}${auditButtons}</div>` : '';
