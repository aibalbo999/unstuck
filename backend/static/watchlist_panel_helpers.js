(function () {
    ...
    function watchlistDailyBoard(items, daily, escapeHtml) {
        ...
        const auditButtons = auditItems.map(item => {
            ...
            const missingFields = Array.isArray(item.missing_quality_fields) ? item.missing_quality_fields : [], missingQualityFields = missingFields.join(','), missingFieldSummary = missingFields.map(field => missingFieldLabels.find(([key]) => key === field)?.[1] || field).filter(Boolean).join('、');
            ...
            const targetContext = [reviewSummary, missingFieldSummary ? `結構化缺口：${missingFieldSummary}` : '', artifactSummary].filter(Boolean).join('；');
