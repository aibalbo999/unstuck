(function () {
    ...
    function watchlistDailyBoard(items, daily, escapeHtml) {
        ...
        const audit = daily?.report_quality_audit || {}, missingQuality = Number(audit.quality_metadata_missing_reports || 0), excludedSnapshots = Number(audit.snapshot_invalid_reports || 0) + Number(audit.snapshot_unverified_reports || 0), coverage = audit.quality_metadata_coverage_pct, coverageLabel = audit.quality_metadata_coverage_basis === 'verified_snapshot_reports' ? '已驗證快照覆蓋' : '覆蓋'; const freshness = audit.decision_freshness_summary || {}; const freshnessAudited = Number(freshness.audited_reports); const freshnessCurrent = Number(freshness.current_reports); const freshnessRerun = Number(freshness.needs_rerun_reports); const freshnessUnknown = Number(freshness.unknown_reports); const freshnessSummary = audit.scope === 'all_indexed_reports' && audit.selection_basis === 'latest_per_ticker_pipeline' && Number.isFinite(freshnessAudited) && freshnessAudited >= 0 && Number.isFinite(freshnessCurrent) && freshnessCurrent >= 0 && Number.isFinite(freshnessRerun) && freshnessRerun >= 0 && Number.isFinite(freshnessUnknown) && freshnessUnknown >= 0 && freshnessCurrent + freshnessRerun + freshnessUnknown === freshnessAudited ? `分析新鮮度：目前一致 ${Math.floor(freshnessCurrent)}、需完整重跑 ${Math.floor(freshnessRerun)}${freshnessUnknown > 0 ? `、無法判定 ${Math.floor(freshnessUnknown)}` : ''}` : ''; const repairQueueSampled = Number(daily?.repair_queue?.summary?.sampled_reports); const repairQueueScopeSummary = audit.scope === 'all_indexed_reports' && Number.isFinite(repairQueueSampled) && repairQueueSampled > 0 ? `修復 queue 範圍：取樣 ${Math.floor(repairQueueSampled)} 份報告` : ''; const repairSampleOverlap = audit.repair_sample_overlap || {}; const repairSampleOverlapStatus = String(repairSampleOverlap.status || '').trim().toLowerCase(); const repairSampleGap = Number(repairSampleOverlap.audit_gap_reports); const repairSampleIn = Number(repairSampleOverlap.audit_gap_reports_in_repair_sample); const repairSampleOutside = Number(repairSampleOverlap.audit_gap_reports_outside_repair_sample); const repairSampleReturned = Number(repairSampleOverlap.audit_gap_items_returned); const repairSampleOverlapSummary = repairSampleOverlapStatus === 'complete' && Number.isFinite(repairSampleGap) && repairSampleGap > 0 && Number.isFinite(repairSampleIn) && Number.isFinite(repairSampleOutside) ? `品質缺口與 repair sample：${Math.floor(repairSampleIn)}/${Math.floor(repairSampleGap)} 在 sample；${Math.floor(repairSampleOutside)} 份不在 sample` : repairSampleOverlapStatus === 'partial' && Number.isFinite(repairSampleGap) && Number.isFinite(repairSampleReturned) && repairSampleGap > repairSampleReturned ? `品質缺口與 repair sample：只載入 ${Math.floor(repairSampleReturned)}/${Math.floor(repairSampleGap)} 份缺口，未展開部分無法判定` : '';
        ...
        const auditParts = freshnessSummary ? [freshnessSummary] : [], auditSummaryItems = freshnessSummary ? [freshnessSummary] : [];
