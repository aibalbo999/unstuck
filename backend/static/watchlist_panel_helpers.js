(function () {
    ...
    function nonNegativeInteger(value) { const number = Number(value); return Number.isFinite(number) && Number.isInteger(number) && number >= 0 ? number : 0; }
    ...
    function watchlistDailyBoard(items, daily, escapeHtml) {
        ...
        const audit = daily?.report_quality_audit || {}, missingQuality = nonNegativeInteger(audit.quality_metadata_missing_reports), excludedSnapshots = nonNegativeInteger(audit.snapshot_invalid_reports) + nonNegativeInteger(audit.snapshot_unverified_reports), coverageValue = Number(audit.quality_metadata_coverage_pct), coverage = Number.isFinite(coverageValue) && coverageValue >= 0 && coverageValue <= 100 ? Math.round(coverageValue * 100) / 100 : null, coverageLabel = audit.quality_metadata_coverage_basis === 'verified_snapshot_reports' ? '已驗證快照覆蓋' : '覆蓋'; const freshnessSummary = window.StockAgentWatchlistFreshnessHelpers?.summary?.(audit) || ''; const repairQueueSummary = daily?.repair_queue?.summary || {}; const repairQueueSampled = Number(repairQueueSummary.sampled_reports); const repairQueueScopeSummary = audit.scope === 'all_indexed_reports' && Number.isFinite(repairQueueSampled) && repairQueueSampled > 0 ? `修復 queue 範圍：取樣 ${Math.floor(repairQueueSampled)} 份報告` : ''; const repairQueueBoundedSummary = window.StockAgentReportQualityQueueScope?.boundedRepairQueueScope?.(repairQueueSummary) || ''; const repairSampleOverlap = audit.repair_sample_overlap || {}; const repairSampleOverlapStatus = String(repairSampleOverlap.status || '').trim().toLowerCase(); const repairSampleGap = Number(repairSampleOverlap.audit_gap_reports); const repairSampleIn = Number(repairSampleOverlap.audit_gap_reports_in_repair_sample); const repairSampleOutside = Number(repairSampleOverlap.audit_gap_reports_outside_repair_sample); const repairSampleReturned = Number(repairSampleOverlap.audit_gap_items_returned); const repairSampleOverlapSummary = repairSampleOverlapStatus === 'complete' && Number.isFinite(repairSampleGap) && repairSampleGap > 0 && Number.isFinite(repairSampleIn) && Number.isFinite(repairSampleOutside) ? `品質缺口與 repair sample：${Math.floor(repairSampleIn)}/${Math.floor(repairSampleGap)} 在 sample；${Math.floor(repairSampleOutside)} 份不在 sample` : repairSampleOverlapStatus === 'partial' && Number.isFinite(repairSampleGap) && Number.isFinite(repairSampleReturned) && repairSampleGap > repairSampleReturned ? `品質缺口與 repair sample：只載入 ${Math.floor(repairSampleReturned)}/${Math.floor(repairSampleGap)} 份缺口，未展開部分無法判定` : '';
        ...
        const returnedItems = Number.isFinite(returnedItemsValue) && returnedItemsValue >= 0 ? Math.floor(returnedItemsValue) : auditItems.length; const totalItemsValue = Number(audit.items_total); const hasBoundedScopeFields = Number.isFinite(totalItemsValue) && totalItemsValue >= 0 && audit.items_returned !== undefined && audit.items_returned !== null; const itemLimit = audit.items_limit === undefined || audit.items_limit === null ? undefined : Number(audit.items_limit); const boundedScopeConsistent = window.StockAgentReportQualityQueueScope?.boundedItemsConsistent; const boundedScopeNeedsConfirmation = hasBoundedScopeFields && typeof boundedScopeConsistent === 'function' && !boundedScopeConsistent(Math.floor(totalItemsValue), returnedItems, audit.items_truncated, itemLimit);
        ...
        const truncationNote = boundedScopeNeedsConfirmation ? `（目前顯示 ${returnedItems}/${Math.floor(totalItemsValue)}；範圍資料需確認）` : audit.items_truncated === true && missingQuality > returnedItems && `（目前顯示 ${returnedItems} 份，另有 ${missingQuality - returnedItems} 份未展開）`;
