(function () {
    ...
    function watchlistDailyBoard(items, daily, escapeHtml) {
        window.StockAgentReportQualityAuditScope?.sanitizeAuditDistributions?.(daily?.report_quality_audit); const queue = daily?.decision_queue || {}, queueItems = Array.isArray(queue.items) ? queue.items : [], top = queueItems[0], total = Number(queue.summary?.total_actionable || 0);
        ...
        const missingPipelineSummary = window.StockAgentReportQualityAuditScope?.pipelineMissingSummary?.(pipelineQuality, missingQuality) || ''; const pipelineContextScopeValid = window.StockAgentReportQualityAuditScope?.pipelineContextScopeValid?.(pipelineQuality, missingQuality) || false; const pipelineContextSummary = missingPipelineSummary && pipelineContextScopeValid ? Object.entries(pipelineQuality).map(([pipeline, summary]) => { const context = rerunContextLabels.map(([key, label]) => { const count = summaryCount(summary?.quality_metadata_missing_by_rerun_context?.[key]); return count !== null && count > 0 ? `${label} ${count}` : ''; }).filter(Boolean).join('、'); return context ? `${pipeline} ${context}` : ''; }).filter(Boolean).join('、') : '';
