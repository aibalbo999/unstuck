(function () {
    function render(audit, escapeHtml) {
        ...
        const summaryItems = [fieldSummary ? `缺口：${fieldSummary}` : '', pipelineSummary ? `模式缺口：${pipelineSummary}` : '', versionSummary ? `版本：${versionSummary}` : '', reviewSummary ? `審核狀態：${reviewSummary}` : '', reviewProgressSummary, provenanceSummary ? `來源：${provenanceSummary}` : ''].filter(Boolean);
