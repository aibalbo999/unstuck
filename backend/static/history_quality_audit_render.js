(function () {
    function render(audit, escapeHtml) {
        ...
        const auditDetails = missing > 0
            ...
            : filteredEmptyLabel ? `<span>目前沒有符合「${e(filteredEmptyLabel)}」的品質 metadata 缺口</span>${basisSummary ? `<em>${e(basisSummary)}</em>` : ''}${currentQualitySummary}` : `<span>符合條件的 ${complete} 份已驗證 snapshot 沒有品質 metadata 缺口</span>${basisSummary ? `<em>${e(basisSummary)}</em>` : ''}${currentQualitySummary}`;
