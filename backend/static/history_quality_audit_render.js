(function () {
    function render(audit, escapeHtml) {
        ...
        const pipelineSummary = Object.entries(pipelineQuality).map(([pipeline, summary]) => {
            ...
            return count !== null && count > 0 ? `${pipeline} ${count}` : '';
        }).filter(Boolean).join('、');
