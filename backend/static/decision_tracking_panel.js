(function () {
    const { trackedGroups, trackedSet, uniqueRecommendedActions } = window.StockAgentDecisionTrackingHelpers;
    ...
    function create(options) {
        ...
        async function runAllRecommendedActions() {
            ...
            try {
                let finished = 0, failed = 0, manualReview = 0;
                ...
                else if (manualReview && finished) notify.success(`已送出 ${finished} 個自動警示動作，${manualReview} 個需人工查看`);
                else if (manualReview) notify.error(`${manualReview} 個警示需人工查看，請開啟報告處理`);
