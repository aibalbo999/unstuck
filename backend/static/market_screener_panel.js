(function () {
    ...
    function triggerReason(item) { const trigger = dailyTrigger(item); return trigger.reason || item.latest_trigger_event?.message || 'Auto-Screener'; }
    ...
    function create(options) {
        ...
        function providersLabel() { return (payload.providers || payload.data_sources || []).join('、') || '-'; }
        function selectedModeValues(ticker, fallbackPipeline) { return selectedModes[ticker] || [fallbackPipeline || 'v4']; }
        function renderBoard(items) {
            const enabled = items.filter(item => item.enabled !== false);
            const modeD = enabled.filter(item => (item.pipeline || '') === 'v4').length;
            return `<div class="market-screener-grid"><span><strong>${escapeHtml(String(enabled.length))}</strong><em>候選股</em></span><span><strong>${escapeHtml(String(modeD))}</strong><em>Mode D</em></span><span><strong>${escapeHtml(providersLabel())}</strong><em>資料源</em></span><span><strong>${escapeHtml(String(payload.updated_at || '-').slice(0, 10))}</strong><em>更新</em></span></div>`;
        }
        function renderModePicker(item) {
            ...
            return `<div class="market-screener-mode-picker" data-screener-mode-picker="${escapeHtml(ticker)}"><div class="market-screener-mode-options">
                ${PIPELINE_OPTIONS.map(option => `<label class="market-screener-mode-option"><input type="checkbox" data-screener-mode="${escapeHtml(ticker)}" value="${escapeHtml(option.value)}" ${values.includes(option.value) ? 'checked' : ''} /><span>${escapeHtml(option.label)}</span><em>${escapeHtml(option.shortLabel)}</em></label>`).join('')}
                </div><button class="maintenance-button market-screener-analyze-button" type="button" data-screener-run-modes="${escapeHtml(ticker)}"><span>排入分析</span></button></div>`;
        }
        function render() {
            ...
            elements.listEl.innerHTML = items.length ? items.map(item => {
                ...
                return `<div class="market-screener-item"><button class="provider-sla-chip market-screener-chip ${item.enabled === false ? 'is-warning' : 'is-ok'} ${ticker === selectedTicker ? 'is-selected' : ''}" type="button" data-screener-select="${escapeHtml(ticker)}" aria-expanded="${ticker === selectedTicker ? 'true' : 'false'}"><strong>${escapeHtml(ticker)}${company ? `<small>${escapeHtml(company)}</small>` : ''}</strong><em>${escapeHtml((item.pipeline || 'v4').toUpperCase())}</em><span>${escapeHtml(tags || 'Auto-Screener')}</span><span>${escapeHtml(triggerReason(item))}</span></button>${renderModePicker(item)}</div>`;
        ...
        async function run() {
            try {
                ...
                await load();
                payload = Object.assign({}, payload, { providers: result.providers || payload.providers, data_sources: result.data_sources || payload.data_sources });
                render();
        ...
        async function runSelectedModes(ticker) {
            ...
            if (!modes.length) { setSummary('請至少選擇一個模式'); return; }
        ...
        function bindEvents() {
            ...
            if (elements.listEl) {
                elements.listEl.addEventListener('click', event => {
                    ...
                    if (runButton) { runSelectedModes(String(runButton.dataset.screenerRunModes || '').toUpperCase()); return; }
                ...
                elements.listEl.addEventListener('change', event => {
                    ...
                    selectedModes[ticker] = Array.from(elements.listEl.querySelectorAll(`[data-screener-mode="${selectorEscape(ticker)}"]:checked`)).map(item => item.value).filter(Boolean);
                });
            }
        }
        function loadOnce() {
