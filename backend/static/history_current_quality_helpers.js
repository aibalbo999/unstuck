(function () {
    ...
    function validated(summary) {
        ...
        const qualityCount = values => ['passed', 'warning', 'blocked', 'unknown'].every(key => Number.isFinite(Number(values[key])) && Number.isInteger(Number(values[key])) && Number(values[key]) >= 0);
        const evidenceCount = ['approved', 'caution', 'rejected', 'unknown'].every(key => Number.isFinite(Number(evidence[key])) && Number.isInteger(Number(evidence[key])) && Number(evidence[key]) >= 0);
        if (summary?.schema_version !== 'report_current_quality_summary.v1' || summary?.scope !== 'historical_filter_current_latest' || summary?.selection_basis !== 'latest_per_ticker_pipeline' || ![audited, nonPassed, total, returned].every(value => Number.isFinite(value) && Number.isInteger(value)) || audited < 0 || nonPassed < 0 || nonPassed > audited || total !== nonPassed || returned < 0 || returned > total || returned !== items.length || !qualityCount(conformance) || !qualityCount(content) || !evidenceCount || Object.values(conformance).reduce((sum, value) => sum + Number(value), 0) !== audited || Object.values(content).reduce((sum, value) => sum + Number(value), 0) !== audited || Object.values(evidence).reduce((sum, value) => sum + Number(value), 0) !== audited) return null;
