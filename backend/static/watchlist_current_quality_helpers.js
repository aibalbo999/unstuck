(function () {
    ...
    function targets(audit, escapeHtml) {
        ...
        const buttons = data.items.map(item => {
            const ticker = e(item.ticker || '報告'), pipeline = e(item.pipeline_id || 'v1'), filename = e(item.filename || ''), status = e(statusLabels[item.report_conformance_status] || '無法判定'), content = e(statusLabels[item.content_credibility_status] || '無法判定'), evidence = e(evidenceLabels[item.evidence_exit_gate_verdict] || '無法判定'), reason = item.reason || '目前品質狀態需要人工查看。', failedCount = Number(item.evidence_failed_count), evidenceFailureSummary = Number.isFinite(failedCount) && failedCount > 0 ? `證據數值不一致 ${Math.floor(failedCount)}` : '', evidenceMismatchFreshness = window.StockAgentReportQualityEvidence?.formatEvidenceMismatchFreshness?.(item.evidence_mismatch_freshness_status, failedCount) || '', evidenceReasonSummary = window.StockAgentReportQualityEvidence?.formatUnverifiableReasonSummary?.(item.evidence_unverifiable_reason_counts) || '', reasonText = [reason, evidenceFailureSummary, evidenceMismatchFreshness, evidenceReasonSummary].filter(Boolean).map(e).join('；');
