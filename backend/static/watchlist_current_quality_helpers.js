(function () {
    ...
    function validated(audit) {
        ...
        const count = values => ['passed', 'warning', 'blocked', 'unknown'].every(key => Number.isFinite(Number(values[key])) && Number(values[key]) >= 0);
        if (summary.schema_version !== 'report_current_quality_summary.v1' || summary.scope !== 'all_indexed_reports' || summary.selection_basis !== 'latest_per_ticker_pipeline' || ![audited, nonPassed, total, returned].every(Number.isFinite) || audited < 0 || nonPassed < 0 || total !== nonPassed || returned < 0 || returned > total || returned !== items.length || !count(conformance) || !count(content) || !['approved', 'caution', 'rejected', 'unknown'].every(key => Number.isFinite(Number(evidence[key])) && Number(evidence[key]) >= 0) || Object.values(conformance).reduce((sum, value) => sum + Number(value), 0) !== audited || Object.values(content).reduce((sum, value) => sum + Number(value), 0) !== audited || Object.values(evidence).reduce((sum, value) => sum + Number(value), 0) !== audited) return null;
    ...
    function summary(audit) {
        ...
        const qualityActionSummary = window.StockAgentReportQualityEvidence?.formatQualityActionSummary?.(data.summary.quality_gate_action_counts) || '';
        const evidenceDetail = [evidenceFailureSummary, evidenceMismatchFreshnessSummary, evidenceReasonSummary, blockerSummary, contentBlockerFreshnessSummary, qualityActionSummary].filter(Boolean).join('；');
