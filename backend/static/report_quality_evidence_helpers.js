(function () {
    ...
    } function formatUnverifiableReasonSummary(counts) { const entries = counts && typeof counts === 'object' && !Array.isArray(counts) ? Object.entries(counts).map(([key, value]) => [String(key || '').trim(), Number(value)]).filter(([key, value]) => key && Number.isFinite(value) && value > 0).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])) : []; return entries.length ? `證據未驗證原因：${entries.map(([key, value]) => `${verificationReasonLabels[key] || key} ${Math.floor(value)}`).join('、')}` : ''; } function formatEvidenceMismatchFreshnessSummary(claims, reports) { const claimEntries = claims && typeof claims === 'object' && !Array.isArray(claims) ? Object.entries(claims).map(([key, value]) => [String(key || '').trim(), Number(value), Number(reports?.[key])]).filter(([key, value]) => key && Number.isFinite(value) && value > 0).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])) : []; return claimEntries.length ? `數值不一致分布：${claimEntries.map(([key, value, reportCount]) => `${evidenceMismatchFreshnessLabels[key] || key} ${Math.floor(value)} 筆${Number.isFinite(reportCount) && reportCount > 0 ? `／${Math.floor(reportCount)} 份` : ''}`).join('、')}` : ''; } function formatEvidenceMismatchFreshness(status, count) { const value = Number(count), key = String(status || '').trim().toLowerCase(); return evidenceMismatchFreshnessLabels[key] && Number.isFinite(value) && value > 0 ? `數值不一致來源：${evidenceMismatchFreshnessLabels[key]} ${Math.floor(value)} 筆` : ''; } function formatBlockerEntries(counts) { const entries = counts && typeof counts === 'object' && !Array.isArray(counts) ? Object.entries(counts).map(([key, value]) => [String(key || '').trim(), Number(value)]).filter(([key, value]) => key && Number.isFinite(value) && value > 0).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])) : []; return entries.map(([key, value]) => `${qualityBlockerLabels[key] || key} ${Math.floor(value)}`).join('、'); } function formatQualityBlockerSummary(conformance, content) { const conformanceText = formatBlockerEntries(conformance), contentText = formatBlockerEntries(content), parts = []; if (conformanceText) parts.push(`報告一致性：${conformanceText}`); if (contentText) parts.push(`內容可信度：${contentText}`); return parts.length ? `品質阻斷來源：${parts.join('；')}` : ''; } function formatContentBlockerFreshnessSummary(reports) { const entries = reports && typeof reports === 'object' && !Array.isArray(reports) ? Object.entries(reports).map(([key, value]) => [String(key || '').trim(), Number(value)]).filter(([key, value]) => key && Number.isFinite(value) && value > 0).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])) : []; return entries.length ? `內容阻斷版本：${entries.map(([key, value]) => `${evidenceMismatchFreshnessLabels[key] || key} ${Math.floor(value)} 份`).join('、')}` : ''; }
    window.StockAgentReportQualityEvidence = { context, fieldLabels, limitation, renderTargetContext, formatUnverifiableReasonSummary, formatEvidenceMismatchFreshnessSummary, formatEvidenceMismatchFreshness, formatQualityBlockerSummary, formatContentBlockerFreshnessSummary };
})();
