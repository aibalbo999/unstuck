document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            if (data.reports && data.reports.length > 0) {
                historyList.innerHTML = data.reports.map(r => `
                    <div class="history-item" data-filename="${escapeHtml(r.filename)}" data-ticker="${escapeHtml(r.ticker)}">
    ...
    historyList.addEventListener('click', (event) => {
        ...
        if (item) {
            openReport(item.dataset.filename, item.dataset.ticker);
        }
    });
    ...
    historyList.addEventListener('keydown', (event) => {
        ...
        const item = event.target.closest('.history-item');
        if (item) openReport(item.dataset.filename, item.dataset.ticker);
    });

    // 開啟報告
    function openReport(filename, ticker) {
    ...
    function connectAnalysisStream(ticker) {
        clearTimeout(reconnectTimer);
        ...
        eventSource.onerror = () => {
            ...
            reconnectTimer = setTimeout(() => connectAnalysisStream(ticker), delay);
        };
    }
