(function () {
    ...
    const dashboardActionItems = payload => (payload?.actions || []).slice(0, 3).map(item => ({ action: item.type === 'rerun_report' ? 'view-report' : (item.type === 'run_watchlist' ? 'run-watchlist' : 'open-ops'), label: item.type === 'rerun_report' ? '查看報告' : (item.type === 'run_watchlist' ? '建立/更新報告' : '查看狀態'), title: item.title || '今日待處理', detail: item.detail || '', filename: item.filename, ticker: item.ticker, pipeline: item.pipeline_id || 'v1' }));
    function create(options) {
        ...
        function renderActions(items) {
            ...
            elements.actionList.innerHTML = `<div class="operator-action-list-header"><strong>今日待處理</strong><span>快速操作</span></div>${items.map(item => `<div class="operator-action-row"><span><strong>${escapeHtml(item.title)}</strong><em>${escapeHtml(item.detail || '')}</em></span><button class="operator-action-button" type="button" data-operator-action="${escapeHtml(item.action)}" data-filename="${escapeHtml(item.filename || '')}" data-ticker="${escapeHtml(item.ticker || '')}" data-pipeline="${escapeHtml(item.pipeline || 'v1')}">${escapeHtml(item.label)}</button></div>`).join('')}`;
        }
        async function load() {
