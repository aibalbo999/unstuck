(function () {
    ...
    function targets(audit, escapeHtml) {
        ...
        const label = window.StockAgentReportQualityQueueScope?.boundedItemsLabel?.('待重跑報告', data.total, data.returned, data.items.items_truncated, data.items.items_limit) || (data.items.items_truncated === true ? `待重跑報告（顯示 ${data.returned}/${data.total}）` : (data.returned < data.total ? `待重跑報告（顯示 ${data.returned}/${data.total}；範圍資料需確認）` : `待重跑報告（${data.total}）`));
