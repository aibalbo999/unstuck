"""Source audit policy assembly for data-fetch payloads."""
...
from data_trust import (
    AUDIT_STATUS_DEGRADED_ENRICHMENT, AUDIT_STATUS_ERROR, AUDIT_STATUS_SKIPPED_FRESH_CACHE,
...
def _append_source_fetch_audit(
    data: dict, source: str, provider: str, status: str, *, fetched_at_epoch: Optional[float] = None,
    started_at_epoch: Optional[float] = None, finished_at_epoch: Optional[float] = None,
...
def _append_full_fetch_audit(
    data: dict, ticker: str, provider_name: str, *, started_at_epoch: Optional[float],
    fetched_at_epoch: Optional[float], skip_optional_http: bool,
) -> dict:
