        ...
        for source, provider, status, duration_ms, record_count, message, timestamp in rows:
            ...
            conn.execute(
                ...
                ON CONFLICT(source, provider) DO UPDATE SET
                    ...
                    not_configured_count = not_configured_count + excluded.not_configured_count,
                    degraded_enrichment_count = degraded_enrichment_count + excluded.degraded_enrichment_count,
                ...
                (
                    ...
                    1 if status == "not_configured" else 0,
                    1 if status == "degraded_enrichment" else 0,
                    max(duration_ms, 0),
                    max(record_count, 0),
                    status,
                    message,
...
def get_provider_sla_summary(limit: int = 100) -> list[dict]:
    ...
    for row in rows:
        ...
        not_configured_count = int(row["not_configured_count"] or 0)
        degraded_enrichment_count = int(row["degraded_enrichment_count"] or 0)
        total_duration_ms = int(row["total_duration_ms"] or 0)
        availability_attempts = max(0, attempts - not_configured_count)
        healthy_count = success_count + skipped_fresh_cache_count + degraded_enrichment_count
        item = {
            "source": row["source"],
            ...
            "degraded_enrichment_count": degraded_enrichment_count,
            "success_rate": round(healthy_count / availability_attempts, 4) if availability_attempts else 1.0,
            "avg_duration_ms": round(total_duration_ms / availability_attempts, 2) if availability_attempts else 0.0,
