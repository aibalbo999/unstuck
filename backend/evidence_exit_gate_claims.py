...
def _path_markers_for_claim(claim: dict[str, Any]) -> tuple[str, ...]:
    ...
    if "last_5_trading_days_net_buy_thousand_shares" in raw_text or label in ("last5tradingdaysnetbuy", "last5daysnetbuy") or (label == "法人合計買超" and "近5個交易日" in _normalize_match_text(f"{claim.get('context_text')} {claim.get('raw_text')}") and "法人" in _normalize_match_text(f"{claim.get('context_text')} {claim.get('raw_text')}")):
        return ("institutional_trading.last_5_trading_days_net_buy_thousand_shares",)
