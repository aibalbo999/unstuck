"""Claim parsing and semantic path hints for the evidence exit gate."""
...
_HORIZON_PREFIX_RE = re.compile(
    r"^\s*(?:近\s*)?\d+(?:\s*[-–—~～至到]\s*\d+)?\s*(?:週|周|weeks?|個月|月|年|years?|天|日|days?)",
    re.IGNORECASE,
)
...
_SNAPSHOT_METADATA_PATH_MARKERS = (
    ...
    "equity_multiplier_note", "reproducibility_packet", "report_conformance", "report_lint", "snapshot_hash", "snapshot_refreshed_at", "source_audit", "target_ticker",
)
...
_FIELD_HINTS: tuple[tuple[tuple[str, ...], tuple[str, ...]], ...] = (
    ...
    (("Return Today", "Borrowed Short Return Today", "Today's borrowed short return"), ("borrowed_short_return_today", "shares_to_lots", "shares_to_millions")), (("Retail", "小戶 (持股 < 50 張) 比例"), ("retail_holders_lt_50_lots_pct",)), (("Holders < 50 lots",), ("retail_holders_lt_50_lots_pct",)), (("Holders > 1000 lots", "Holders > 1,000 lots", "Major > 1000 lots", "Major > 1,000 lots"), ("major_holders_gt_1000_lots_pct",)), (("Equity Multiplier", "權益乘數"), ("equity_multiplier",)),
    (("大戶", "major holders", "major_holders"), ("major_holders", "major_holders_gt_1000_lots_pct")), (("散戶", "retail holders", "retail_holders"), ("retail_holders", "retail_holders_lt_50_lots_pct")), (("Margin Previous Balance",), ("margin_previous_balance",)), (("融資餘額", "margin balance", "margin_balance"), ("margin_balance", "margin_previous_balance")), (("融券餘額", "short balance", "short_balance"), ("short_balance", "short_previous_balance")), (("融資買進", "margin purchase", "margin_purchase"), ("margin_purchase",)), (("融資賣出", "margin sale", "margin_sale"), ("margin_sale",)), (("融券買進", "short purchase", "short_purchase"), ("short_purchase",)), (("融券賣出", "short sale", "short_sale"), ("short_sale",)), (("借券賣出餘額", "borrowed short sale balance", "borrowed_short_sale_balance"), ("borrowed_short_sale_balance",)), (("當日借券還券", "今日借券償還", "borrowed short return today", "today's borrowed short return"), ("borrowed_short_return_today", "shares_to_lots")), (("Dealer", "自營商"), ("institutional_trading.net_buy_thousand_shares_by_category.dealer",)), (("Foreign", "外資"), ("institutional_trading.net_buy_thousand_shares_by_category.foreign",)), (("Investment Trust", "投信"), ("institutional_trading.net_buy_thousand_shares_by_category.investment_trust",)), (("Total Net Buy (30 days)", "totalnetbuy30days"), ("institutional_trading.total_net_buy_thousand_shares",)), (("近 5 個交易日淨買超 (千股)",), ("institutional_trading.last_5_trading_days_net_buy_thousand_shares",)), (("Taiwan Weighted Index",), ("global_market_context.items[twii].latest",)), (("USD/TWD",), ("global_market_context.items[twdx].latest",)), (("WTI Crude Oil", "WTI Crude Oil Futures"), ("global_market_context.items[clf].latest",)), (("US 10Y Yield", "US 10Y Treasury Yield"), ("global_market_context.items[tnx].latest",)), (("VIX",), ("global_market_context.items[vix].latest",)), (("US CPI YoY", "CPI YoY", "美國 CPI 年增率", "CPI年增率"), ("macro_indicators.indicators.us_cpi_yoy.value",)), (("杜邦", "dupont"), ("dupont_identity_note",)),
    (("股價", "現價", "目前價格", "當前價格", "當前市價", "當前價位", "當前報價", "currentprice", "current_price", "Price"), ("current_price", "regularmarketprice", "stock_price", "share_price")),
    (("forwardpe", "forward pe"), ("forward_pe", "forwardpe", "forward_eps")), (("epsimpliedrevenuegrowth", "impliedrevenuegrowth"), ("forward_eps_implied_revenue_growth_pct", "implied_revenue_growth_pct")), (("incomegrowthlatestannual", "latest annual net income growth"), ("latest_annual_net_income_growth",)), (("Latest annual growth",), ("latest_annual_revenue_growth",)), (("CAGR (5yr)",), ("revenue_cagr_5yr",)),
    (("淨利率", "profitmargin", "profit_margin"), ("profit_margin", "profit_margin_raw")), (("熊市", "基本", "牛市", "情境"), ("price_target", "price_targets", "target_price", "scenario", "scenarios")), (("週目標", "weektarget"), ("parsed.trade_setup.target_price", "structured_outputs.24.target_price")),
    ...
    (("市值", "marketcap", "market_cap"), ("market_cap", "marketcap")), (("流動比率", "current ratio", "currentratio"), ("current_ratio",)), (("債務權益比", "權益負債比率", "debt to equity", "debt-to-equity", "debtequity"), ("debt_to_equity",)),
...
def extract_numeric_claims(markdown: str) -> list[dict[str, Any]]:
    ...
    for line_number, raw_line in enumerate((lines := str(markdown or "").splitlines()), start=1):
        ...
        for match in list(_KV_RE.finditer(line)) + list(_TABLE_CELL_RE.finditer(line)):
            ...
            if any(_normalize_match_text(marker) in _normalize_match_text(label) for marker in ("支撐", "壓力", "高點", "低點", "週高低")):
                for secondary in _SECONDARY_EVIDENCE_RE.finditer(line[match.end():]):
                    secondary_number = _clean_number(secondary.group("num"))
                    secondary_label = f"{label}（次要價位）"
                    ...
                    secondary_text = secondary.group(0).strip()
        ...
        for series_match in series_matches[1:]:
            series_label = f"{series_match.group('month')}/{series_match.group('day')}"
            series_number = _clean_number(series_match.group("num"))
            series_key = (series_label, round(series_number, 6), line_number) if series_number is not None else None
            if series_number is None or not _valid_claim_number(series_number) or series_key in seen:
                continue
            seen.add(series_key)
            claims.append({"id": len(claims) + 1, "label": series_label, "reported_value": series_number, "unit": "", "line_number": line_number, "raw_text": line, "context_text": "\n".join(lines[max(0, line_number - 3):line_number - 1]) if line_number > 1 else ""})
...
def _path_markers_for_claim(claim: dict[str, Any]) -> tuple[str, ...]:
    claim_text = str(claim.get("raw_text") or "")
    raw_label = str(claim.get("label") or "").lower()
    series_context = str(claim.get("series_context_text") or "")
    label = _normalize_match_text(raw_label)
    if not label:
        return ()
    if (date_match := re.fullmatch(r"(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2})", raw_label.strip(), re.IGNORECASE)) and any(_normalize_match_text(marker) in _normalize_match_text(series_context) for marker in ("daily_total_net_buy_last_10", "Last 10 trading days daily total net buy", "Last 10 trading days daily net buy", "Daily Net Buy (Last 10 days)", "Last 10 days daily trend", "Daily Trend (Last 10 days)", "Last 10 days daily total net buy", "Recent Trend (Last 10 days)")):
    ...
    if "factset" in raw_text:
        return ("factset",)
    if any(marker in raw_text for marker in _NORMALIZED_RESEARCH_CONTEXT_MARKERS) or ("sp500" in raw_text and "台股加權指數" in raw_text and "change1d" in raw_text) or (label == "sp500" and str(claim.get("unit") or "").lower() == "%" and re.search(r"\b1d\b", claim_text, re.IGNORECASE)):
        return ("broker_research",) if any(marker in raw_text for marker in _NORMALIZED_RESEARCH_CONTEXT_MARKERS) else ("global_market_context.items[spy].change_1d_pct",)
    indexed_match = re.search(r"global[_ ]?market[_ ]?context\[(?P<index>\d+)\]\.change[_ ]?(?P<days>[15])d[_ ]?pct", claim_text, re.IGNORECASE)
    indexed_label = re.search(r"\[(?P<index>\d+)\].*?change[_ ]?(?P<days>[15])d[_ ]?pct", raw_label, re.IGNORECASE)
    symbol_matches = list(re.finditer(r"\(([A-Z][A-Z0-9^=.-]{1,9})\)", claim_text[:indexed_match.start()])) if indexed_match else []
    if indexed_match and indexed_label and symbol_matches:
        return (f"global_market_context.items[{_normalize_match_text(symbol_matches[-1].group(1))}].change_{indexed_label.group('days')}d_pct",)
    ...
    if ("1000lots" in raw_text and "concentration" in label) or ("50lots" in raw_text and "retail" in label):
    ...
    date_series_match = re.search(r"(?P<month>\d{1,2})/(?P<day>\d{1,2})", raw_label)
    date_series_context = str(claim.get("context_text") or "")
    context_years = set(re.findall(r"(20\d{2})\s*年", date_series_context))
    month = int(date_series_match.group("month")) if date_series_match else 0
    candidate_years = {month_name[:4] for month_name in claim.get("_price_history_months") or () if int(month_name[5:7]) == month}
    if "觀察近三個月價格" in claim_text and date_series_match and len(context_years) == len(candidate_years) == 1 and context_years == candidate_years:
        return (f"price_history[{next(iter(context_years))}-{month:02d}-{int(date_series_match.group('day')):02d}]",)
    ...
    if (dated_level := re.search(r"(20\d{2})\s*[-/年.]\s*(\d{1,2})\s*[-/月.]\s*(\d{1,2})", claim_text)) and any(_normalize_match_text(marker) in label for marker in ("支撐", "壓力")) and re.search(r"(?:NT\$|\$|TWD|元)", claim_text, re.IGNORECASE) and any(marker in claim_text for marker in ("價格", "月底價", "前高", "低點", "高點", "收盤")) and not any(marker in re.split(r"[)）\n。；;]", claim_text[dated_level.start():], maxsplit=1)[0] for marker in ("market_catalysts", "catalyst", "新聞", "news", "催化劑")) and (previous_numbers := list(_NUMBER_IN_STRING_RE.finditer(claim_text[:dated_level.start()]))) and (re.fullmatch(r"\s*(?:NT\$|\$|TWD|元)?\s*[（(]\s*", claim_text[previous_numbers[-1].end():dated_level.start()]) or re.search(r"此為\s*$", re.sub(r"[*_`]", "", claim_text[previous_numbers[-1].end():dated_level.start()]))) and _clean_number(previous_numbers[-1].group()) == float(claim.get("reported_value") or 0):
        return (f"price_history[{dated_level.group(1)}-{int(dated_level.group(2)):02d}-{int(dated_level.group(3)):02d}]",)
    history_date = re.search(r"(20\d{2})\s*[-/年.]\s*(\d{1,2})\s*[-/月.]\s*(\d{1,2})", claim_text)
    has_price_label = any(_normalize_match_text(marker) in label for marker in ("高點", "低點", "收盤", "支撐", "壓力", "底部", "股價", "價格", "close", "high", "low"))
    has_close_marker = any(_normalize_match_text(marker) in _normalize_match_text(claim_text) for marker in ("收盤", "close", "closing"))
    has_price_unit = bool(re.search(r"(?:NT\$|\$|TWD|元)", claim_text, re.IGNORECASE))
    has_inline_extremum = bool(re.search(r"20\d{2}\s*[-/年.]\s*\d{1,2}\s*[-/月.]\s*\d{1,2}\s*(?:高點|低點|high|low)", claim_text, re.IGNORECASE))
    has_dated_extremum = (any(_normalize_match_text(marker) in label for marker in ("高點", "低點")) or (has_price_label and has_inline_extremum)) and not any(_normalize_match_text(marker) in label for marker in ("52週", "52week"))
    has_news_source = any(marker in raw_text for marker in ("market_catalysts", "catalyst", "新聞", "催化劑", "盤中速報", "news"))
    ...
    if (month_end := re.search(r"(?:(20\d{2})\s*(?:[-/年.]\s*)?)?(\d{1,2})\s*(?:(?:月份?|月)\s*收盤(?:平台|價)?|(?:月|月份)\s*(?:底|末)[^\n]{0,12}(?:低點|高點|收盤(?:平台|價)?))", claim_text, re.IGNORECASE)) and any(_normalize_match_text(marker) in label for marker in ("支撐", "壓力", "高點", "低點", "前高")) and not has_news_source:
        ...
        if previous_numbers and _clean_number(previous_numbers[-1].group()) == float(claim.get("reported_value") or 0) and year and f"{year}-{month:02d}" in candidates:
            return (f"price_history[month-end={year}-{month:02d}]",)
    if (month_extremum := re.search(r"(?:(?P<year>20\d{2})\s*(?:[-/年.]\s*)?(?P<month_with_year>\d{1,2})|(?P<month_without_year>\d{1,2})\s*(?:月|月份))\s*[^\n]{0,16}(?P<kind>低點|高點)", claim_text, re.IGNORECASE)) and any(_normalize_match_text(marker) in label for marker in ("支撐", "壓力", "高點", "低點")) and not has_news_source and (month_value := int(month_extremum.group("month_with_year") or month_extremum.group("month_without_year"))) and (month_years := {month_extremum.group("year")} if month_extremum.group("year") else {month_name[:4] for month_name in claim.get("_price_history_months") or () if int(month_name[5:7]) == month_value}) and len(month_years) == 1 and (previous_numbers := list(_NUMBER_IN_STRING_RE.finditer(claim_text[:month_extremum.start()]))) and _clean_number(previous_numbers[-1].group()) == float(claim.get("reported_value") or 0):
        return (f"price_history[month={next(iter(month_years))}-{month_value:02d}].{'low' if month_extremum.group('kind') == '低點' else 'high'}",)
    if label == "day" and re.search(r"(?<!\d)5\s*-\s*day\s*:", claim_text, re.IGNORECASE) and re.search(r"net\s*buy", claim_text, re.IGNORECASE):
        return ("institutional_trading.last_5_trading_days_net_buy_thousand_shares",)
    if (label == "totalnetbuythousandshares" and "total_net_buy_thousand_shares" in raw_text) or (label == "total" and all(marker in _normalize_match_text(claim.get("context_text")) for marker in ("foreign", "investmenttrust"))):
        return ("institutional_trading.total_net_buy_thousand_shares",)
    if "last_5_trading_days_net_buy_thousand_shares" in raw_text or label in ("last5tradingdaysnetbuy", "last5daysnetbuy") or (label == "法人合計買超" and "近5個交易日" in _normalize_match_text(f"{claim.get('context_text')} {claim.get('raw_text')}") and "法人" in _normalize_match_text(f"{claim.get('context_text')} {claim.get('raw_text')}")) or (label == "trust" and "netbuy30days" in _normalize_match_text(claim_text) and re.search(r"\bforeign\b.*\bdealer\b.*\btrust\b", claim_text, re.IGNORECASE)):
        return ("institutional_trading.net_buy_thousand_shares_by_category.investment_trust",) if label == "trust" else ("institutional_trading.last_5_trading_days_net_buy_thousand_shares",)
    if (label in ("週高點", "週低點", "壓力位", "支撐位", "近期壓力", "關鍵壓力位") or any(_normalize_match_text(marker) in label for marker in ("壓力", "支撐", "防線"))) and str(claim.get("unit") or "").lower() in ("twd", "元") and (week_match := next((match for match in re.finditer(r"(?:(?:52\s*週|52週)\s*(?P<after>最高|最低|高|低)(?:點|價)?\s*[:：為=]?\s*[*_`]*\s*(?:NT\$|\$)?(?P<after_num>-?\d[\d,]*(?:\.\d+)?)|(?P<before_num>-?\d[\d,]*(?:\.\d+)?)\s*(?:TWD|元)?\s*[*_`]*\s*[（(]?\s*[。．]?\s*(?:此為|為|是|目前(?:處於|為|價位即為)?)?\s*(?:52\s*週|52週)\s*(?P<before>最高|最低|高|低)(?![點價]?\s*(?:(?:[:：=])|為|是|上方)?\s*-?\d[\d,]*(?:\.\d+)?)(?:點|價)?(?!\s*(?:(?:[:：=])|為|是|上方)?\s*-?\d[\d,]*(?:\.\d+)?))", str(claim.get("raw_text") or ""), re.IGNORECASE) if _clean_number(match.group("after_num") or match.group("before_num")) == float(claim.get("reported_value") or 0)), None)):
    ...
    if (source_parenthesized_match := re.search(r"`?(?:data\.)?(?:market_data\.)?week_52_(?P<kind>high|low)_twd`?\s*[（(]\s*(?:NT\$|\$)?\s*(?P<num>-?\d[\d,]*(?:\.\d+)?)\s*(?:TWD|元)?\s*[)）]", str(claim.get("raw_text") or ""), re.IGNORECASE)) and _clean_number(source_parenthesized_match.group("num")) == float(claim.get("reported_value") or 0):
        return ("week_52_high",) if source_parenthesized_match.group("kind").lower() == "high" else ("week_52_low",)
    if (label == "low" and re.search(r"52\s*[- ]?week\s+high\s*[:：].*[/,]\s*low\s*[:：]", claim_text, re.IGNORECASE)) or (label in ("週高低", "週高低次要價位") and "52週高低" in _normalize_match_text(f"{claim.get('raw_text')} {claim.get('secondary_context_text')}")):
        return ("week_52_low",) if label in ("low", "週高低次要價位") else ("week_52_high",)
    if label == "最新餘額":
        return ("margin_balance",) if ("融資餘額" in raw_text or "marginbalance" in raw_text or "融資餘額" in _normalize_match_text(claim.get("context_text"))) else ("short_balance",) if ("融券餘額" in raw_text or "shortbalance" in raw_text or "融券餘額" in _normalize_match_text(claim.get("context_text"))) else ()
    if (("riverchart" in raw_text or "河流圖" in raw_text) and str(claim.get("unit") or "").lower() in ("twd", "元") and re.search(r"\d[\d,]*(?:\.\d+)?\s*x\s*(?:區間|位階|band)", claim_text, re.IGNORECASE)) or (label == "x中高分位帶" and (band_match := re.search(r"(?P<multiple>\d[\d,]*(?:\.\d+)?)\s*x", str(claim.get("raw_text") or ""), re.IGNORECASE))) or (label in ("關鍵壓力", "關鍵壓力位") and str(claim.get("unit") or "").lower() in ("twd", "元") and any(marker in raw_text for marker in ("52週最高價", "week52high"))):
        return ("pe_river_chart.bands",) if ("riverchart" in raw_text or "河流圖" in raw_text) else (f"pe_river_chart.bands.{band_match.group('multiple')}x",) if label == "x中高分位帶" else ("week_52_high",)
    if any(_normalize_match_text(marker) in label for marker in ("券資比", "margin short ratio", "short margin ratio")) or (has_news_source and any(_normalize_match_text(marker) in label for marker in ("支撐", "壓力", "關卡", "風險"))) or any(marker in raw_text for marker in ("熊市", "牛市")):
