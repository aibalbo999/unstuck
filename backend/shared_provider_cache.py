...
def shared_fetch(key: str, fetch: Callable, *, freshness_seconds: int,
                 ...
                 use_cache: bool = True, result_ttl: Callable | None = None) -> tuple[object, dict]:
    ...
    with _LOCKS[int(digest[:8], 16) % len(_LOCKS)], _process_lock(digest) as acquired:
        if not acquired:
    ...
    if isinstance(backend, LocalRedisCache):
        try:
            lock = backend._redis.lock(backend._cache_key("provider_lock:" + digest),
                                       timeout=900, blocking_timeout=30)
