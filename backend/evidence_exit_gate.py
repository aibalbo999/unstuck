...
_FIELD_HINTS: tuple[tuple[tuple[str, ...], tuple[str, ...]], ...] = (
    ...
    (("風險", "支撐", "壓力", "關卡"), ("risk_price",)), (("weekhigh", "52 week high"), ("week_52_high",)), (("weeklow", "52 week low"), ("week_52_low",)),
...
def _path_markers_for_claim(claim: dict[str, Any]) -> tuple[str, ...]:
    ...
    history_date = re.search(r"(20\d{2})\s*[-/年.]\s*(\d{1,2})\s*[-/月.]\s*(\d{1,2})", claim_text); has_price_label = any(_normalize_match_text(marker) in label for marker in ("高點", "低點", "收盤", "支撐", "壓力", "股價", "價格", "close", "high", "low")); has_close_marker = any(_normalize_match_text(marker) in _normalize_match_text(claim_text) for marker in ("收盤", "close", "closing")); has_price_unit = bool(re.search(r"(?:NT\$|\$|TWD|元)", claim_text, re.IGNORECASE)); has_dated_extremum = any(_normalize_match_text(marker) in label for marker in ("高點", "低點")) and not any(_normalize_match_text(marker) in label for marker in ("52週", "52week")); has_news_source = any(marker in raw_text for marker in ("market_catalysts", "catalyst", "新聞", "news"))
