...
def _path_markers_for_claim(claim: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (dated_level := re.search(r"(20\d{2})\s*[-/年.]\s*(\d{1,2})\s*[-/月.]\s*(\d{1,2})", claim_text)) and any(_normalize_match_text(marker) in label for marker in ("支撐", "壓力")) and re.search(r"(?:NT\$|\$|TWD|元)", claim_text, re.IGNORECASE) and any(marker in claim_text for marker in ("價格", "月底價", "前高", "低點", "高點", "收盤")) and not any(marker in raw_text for marker in ("market_catalysts", "catalyst", "新聞", "news", "催化劑")) and (previous_numbers := list(_NUMBER_IN_STRING_RE.finditer(claim_text[:dated_level.start()]))) and re.fullmatch(r"\s*(?:NT\$|\$|TWD|元)?\s*[（(]\s*", claim_text[previous_numbers[-1].end():dated_level.start()]) and _clean_number(previous_numbers[-1].group()) == float(claim.get("reported_value") or 0): return (f"price_history[{dated_level.group(1)}-{int(dated_level.group(2)):02d}-{int(dated_level.group(3)):02d}]",)
