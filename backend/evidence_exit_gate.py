...
def evaluate_report_evidence(
    ...
    *,
    ...
    claims = [{**claim, "_price_history_months": price_history_months, "_legacy_conclusion_context_missing": isinstance(snapshot.get("rerun_context"), dict) and not snapshot["rerun_context"].get("parsed") and not snapshot["rerun_context"].get("structured_outputs") and (any(_normalize_match_text(marker) in _normalize_match_text(claim.get("label")) for marker in ("短期目標", "中期目標", "長期目標", "個月目標", "長期潛力")) or (re.fullmatch(r"(?:(?:買入|持有|避免|放空|觀望)[；;:：]\s*)?(?:3|6|12)\s*個月", str(claim.get("label") or "").strip()) and "最終投資建議" in _normalize_match_text(claim.get("raw_text"))))} for claim in extract_numeric_claims(markdown)]
...
def _check_claim(claim: dict[str, Any], snapshot_values: list[dict[str, Any]], *, tolerance_pct: float) -> dict[str, Any]:
    ...
    path_markers = _path_markers_for_claim(claim); raw_claim_text = str(claim.get("raw_text") or ""); news_boundary = bool(re.search(r"(?:market[_ ]?catalysts?|recent[_ ]?catalysts?|新聞|催化劑|news)", raw_claim_text, re.IGNORECASE) and any(_normalize_match_text(marker) in _normalize_match_text(claim.get("label")) for marker in ("支撐", "壓力", "關卡", "風險"))); research_boundary = any(marker in path_markers for marker in ("factset", "broker_research")); legacy_conclusion_boundary = bool(claim.get("_legacy_conclusion_context_missing"))
