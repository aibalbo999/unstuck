...
def normalize_ops_queue_payload(queue: Any) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "depth": safe_int(source.get("depth")),
        "queues": {
            safe_text(name).strip() or "unknown": _named_queue_detail(details)
            for name, details in safe_dict(source.get("queues")).items()
        },
    }
...
def _copy_queue_supplemental_fields(payload: dict[str, Any], source: dict[str, Any]) -> None:
    ...
    if "active_tasks" in source:
        payload["active_tasks"] = safe_int(source.get("active_tasks"))
    ...
    if "job_timeout_seconds" in source:
        payload["job_timeout_seconds"] = safe_int(source.get("job_timeout_seconds"))
    if "error" in source:
...
def _registry_counts(registries: Any) -> dict[str, int]:
    return {safe_text(registry).strip() or "unknown": safe_int(count) for registry, count in safe_dict(registries).items()}
