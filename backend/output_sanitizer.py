...
class SecureOutputSanitizer:
    ...
    def __init__(
        ...
        *,
        ...
        guardrail: GuardrailHook | None = None,
    ):
    ...
    async def sanitize(self, text: str) -> str:
        ...
        raw = self._validate_payload(text if text is not None else "")
        guarded = await self._apply_guardrail(raw)
        cleaned = sanitize_model_output(guarded)
        cleaned = self.redact_pii(cleaned)
        cleaned = self._validate_payload(cleaned)
    ...
    async def _apply_guardrail(self, text: str) -> str:
        if self.guardrail is None:
            return text
        return await apply_external_guardrail(text, self.guardrail, stage="output")
