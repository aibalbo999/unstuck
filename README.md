...
- `SQLITE_BACKUP_RETENTION_DAYS`：受管 SQLite 每日備份保留的 UTC 日曆日數，預設 `3`
...
`sqlite-maintenance` 預設只會 dry-run，回報預計移除的逾期備份但不刪檔；加上 `--write` 才會刪除候選檔，並對 runtime SQLite DB 建立每日備份、執行 WAL checkpoint 與 `VACUUM`。每日備份以 `SQLITE_BACKUP_DIR` 為目錄，同一天重跑會沿用既有備份。輪替依本輪 UTC 日期計算 cutoff；預設保留當日與前 2 個日曆日，且只管理 `cache_db-YYYYMMDD.sqlite3`、`task_db-YYYYMMDD.sqlite3`、`checkpoint_db-YYYYMMDD.sqlite3`。`manual-archive.sqlite3` 等未知檔名、目錄與其他檔案不會被刪除。
