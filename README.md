...
scripts/maintenance.sh cleanup-failed-queue
...
`cleanup-failed-queue` 預設只會盤點 RQ failed registry 中可由 `ended_at`/`created_at` 判定超過 7 天的任務；加上 `--write` 才會刪除 stale failed job 與其 job data。沒有可驗證時間的任務會保留，不會自動重試或清理近期失敗。
維護面板的四個清理按鈕（報告索引、任務紀錄、來源健康紀錄、過期失敗任務）都會先執行 `write=false` 預覽；只有候選數大於零且操作員在共用確認視窗核准後，才會呼叫對應的 `write=true` 清理。取消、沒有候選或確認器不可用時不會寫入。
