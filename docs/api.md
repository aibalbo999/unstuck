...
For missing metadata after a refresh, `items[]` may include `rerun_context_status=present|partial|missing|artifact_fallback_available`, plus `snapshot_rerun_context_status` and `artifact_rerun_context_status` when the corresponding evidence was checked. The snapshot status describes persisted analyses, structured outputs, and parsed recommendation; `artifact_fallback_available` means the snapshot context is incomplete but the Markdown artifact contains all required preceding Agent sections for the existing partial-rerun fallback. This is a read-only availability signal, not a gate result; it does not enqueue a rerun, approve a report, or change `blocks_auto_rerun`.
