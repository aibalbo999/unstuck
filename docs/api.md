...
Scenario target labels `熊市情境`、`基本情境`、`牛市情境` and `熊/基/牛情境` are classified as `scenario_target_not_canonical` when no canonical scenario scalar exists. Quality metadata, DCF values, current price, or another target path are not borrowed; the claim remains `unverifiable`. A real canonical scenario field still uses ordinary matched/mismatch evaluation.

Exact narrative level labels `心理關卡`、`第二支撐` and `關鍵支撐區` are classified as `technical_level_not_canonical` when no canonical technical-level or `risk_price` scalar exists. The gate does not borrow current price, target candidates, or nearby history; a canonical `risk_price` still uses ordinary matched/mismatch evaluation.
