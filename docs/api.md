...
The content-credibility evidence-matrix check does not treat a matching `最終投資建議` claim name as sufficient coverage: the row must have a usable status (`success`, `skipped_fresh_cache`, or `degraded_enrichment`) and a non-empty basis. Otherwise it records `unusable_final_recommendation_evidence` as a warning; numeric evidence approval remains the independent responsibility of `evidence_exit_gate`.
