...
For mode-D trade setup checks, a target or stop-loss field containing multiple non-range scenario prices records `ambiguous_trade_setup_price_inputs` as a non-blocking warning and exposes the parsed `target_price_candidates` or `stop_loss_candidates` in the check details. An explicit two-endpoint price range remains valid and is not treated as ambiguous. Neutral direction remains neutral; the warning only prevents a multi-scenario string from being presented as a confidently checked single price.
