...
For missing metadata after a refresh, `items[]` may include `rerun_context_status=present|partial|missing`. This is a read-only availability signal for the original analyses, structured outputs, and parsed recommendation used by a partial rerun; `missing` means the operator should arrange a complete rerun when the snapshot is marked `needs_rerun`. It does not enqueue a rerun, approve a report, or change `blocks_auto_rerun`.
