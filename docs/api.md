...
The exact `Latest annual growth` label is treated as revenue growth and uses `data.latest_annual_revenue_growth`. It does not borrow `data.latest_annual_net_income_growth` when the values happen to match; a mismatch remains on the revenue path, and the label stays unverifiable when the canonical revenue value is unavailable.

Chinese institutional aliases are context-bound: `近 5 個交易日淨買超 (千股)` maps to `data.institutional_trading.last_5_trading_days_net_buy_thousand_shares`, while `法人合計買超` maps there only when the surrounding text explicitly establishes a five-day window. `小戶 (持股 < 50 張) 比例` maps to the TDCC retail-holder field, and `權益負債比率` maps to `data.debt_to_equity`. A standalone `S&P 500` percentage claim maps to `data.global_market_context.items[spy].change_1d_pct` only when it explicitly says `1d`; index targets do not use that daily-change path. These mappings do not borrow same-value total, major-holder, current-ratio, or other market-symbol fields.
The short `Trust` alias maps to `data.institutional_trading.net_buy_thousand_shares_by_category.investment_trust` only when the same raw line explicitly establishes `Net Buy (30 days)` and lists `Foreign`, `Dealer`, and `Trust` in the institutional classification; an isolated `Trust` remains unverifiable instead of borrowing another category or same-valued field.
...
Current-quality, historical-audit, provider-impact, decision-queue, dashboard, and watchlist-alert payloads also use that explicit boolean contract for `blocks_auto_rerun`, free-mode flags, and freshness rerun flags. Legacy strings such as `false` remain false, so an operator action cannot be falsely blocked, promoted, or hidden by generic truthiness.
...
The content-credibility projection selects the first non-missing pipeline identity from `rerun_context.pipeline_id` and then snapshot `pipeline`; placeholders such as `N/A`, `NULL`, and empty text do not override a usable snapshot pipeline. Therefore a v4 snapshot with a placeholder rerun pipeline still runs the v4 trade-setup alignment rules. When neither source has a usable pipeline identity, the projection remains unavailable instead of defaulting the missing historical context to v1.

Report rendering and conformance use the same placeholder-aware pipeline rule. A usable request pipeline takes precedence over a usable context pipeline; `N/A`, `NULL`, and empty request/context values do not overwrite a valid v4 identity from the other source. The renderer writes the selected identity to each generated snapshot and metadata envelope, while conformance uses the snapshot pipeline when context pipeline metadata is missing. This prevents a v4 report from being rendered or checked with v1 mode headings because of a placeholder field.

Full and final report reruns resolve the source pipeline with the same evidence boundary: a usable snapshot `pipeline` takes precedence, and `N/A`, `NULL`, or empty snapshot metadata falls back to the pipeline encoded in the requested report filename. The resolver does not write the snapshot or infer a mode from another report; this keeps a v2 or v4 rerun on its filename's Agent sequence when legacy snapshot metadata is unavailable.

Report-history row hydration and artifact rerun-context auditing use the shared `report_pipeline_identity.resolve_report_pipeline_id()` resolver as well. A usable stored/index pipeline takes precedence, followed by snapshot metadata and then the filename; placeholder values cannot make a v4 preview, quality audit bucket, or v2 artifact Agent-sequence check fall back to v1.
...
The browser evidence helper trims and lowercases `reason_codes` before matching `quality_metadata_before_refresh` and `quality_metadata_after_refresh`, so formatting drift cannot hide refresh attribution in preview, daily targets, or historical targets. The stored reason codes remain unchanged.

Generated report data-trust summaries use the same bounded reason policy: HTML exposes at most four and Markdown at most eight reason labels, with concrete source error/stale/provider SLA reasons before generic freshness or note reasons, while equal-priority codes retain their original order. This changes only reason visibility in the generated report and does not change trust status or persisted reason codes.
...
All report-facing quality and decision targets that carry a filename use the shared `report_pipeline_identity.resolve_report_pipeline_id()` boundary. A valid stored/index or snapshot identity wins; `N/A`, `NULL`, and blank values fall back to the filename, while a missing or unparseable filename keeps the existing v1 default. This applies to current-quality, audit, freshness, provider-impact, rerun, backtest, and daily decision projections and changes only read-only display/routing identity, not gate, snapshot, artifact, review, queue, or rerun state.

Backtest results use the same boundary when they are evaluated or persisted: a placeholder pipeline cannot be written into the canonical operational ledger when the report filename identifies a valid mode. Outcome calibration and its daily decision projection read that canonical identity, so historical hit/miss counts and quality attribution remain grouped under the report's actual pipeline without rewriting report artifacts.

Historical quality-audit version labels use the same filename-aware identity when building the latest-per-ticker/pipeline map and annotating each indexed version. A legacy placeholder therefore remains `current` or `historical` under the valid pipeline parsed from its report filename instead of becoming `unknown`; this changes audit classification only.

The daily dashboard's read-only `report_quality_audit.repair_sample_overlap` comparison also canonicalizes the pipeline on both the sampled report row and audit item before matching `(filename, pipeline)`. This keeps a legacy `N/A` sample row aligned with the v4 audit item and prevents `audit_gap_reports_outside_repair_sample` from being overstated.

The report index list query applies the same filename-aware pipeline identity before filtering by `pipeline` and before selecting `latest_per_ticker_pipeline`. A legacy row with a valid v4 filename and stored `N/A` is therefore included in v4 history results and cannot form a separate latest group from another v4 row for the same ticker. Recommendation, data-trust, search, total, and pagination boundaries remain read-only; the query does not rewrite legacy index rows or artifacts.

The data-only refresh endpoint preserves that identity when it writes a new snapshot: it resolves the pipeline from the report filename when the previous snapshot stores `N/A`, `NULL`, or blank. The canonical pipeline is used for refresh context, provisional diff snapshot, and final snapshot, so refreshing data cannot silently change a v4 report into a placeholder or v1 context. This does not mean the report conclusion was rerun; the existing freshness and `needs_rerun` contract remains authoritative.
...
An explicit week-52 source citation such as `56.4 元（引用 \`market_data.week_52_high_twd\`）` or its `來源`/`source` equivalent may also bind to `data.week_52_high`/`data.week_52_low`. The citation matcher tolerates Markdown emphasis between the value and source marker, but still requires the same reported value and the explicit canonical source name; a generic pressure/support label or same-valued `risk_price` remains `unverifiable`.
...
Borrowed-short claims map to their own TWSE paths: explicit `Borrowed Short Sale Today` uses `data.chip_data.twse_margin_short_sales.borrowed_short_sale_today`, while explicit `Borrowed Short Return Today`/`Today's borrowed short return`/`Return Today`, or a compact `return` after `borrowed short sale`, uses `data.chip_data.twse_margin_short_sales.borrowed_short_return_today`. When a flow is displayed in `k`, `張`, or `M`, raw shares are converted only to that claim's stated unit. The exact `vs Sale Today` comparison label remains scoped to `borrowed_short_sale_today`; the parser does not reuse the sale path for a return claim or the return path for `vs Sale Today`.
...
Historical quality targets expose `report_quality_revision` and a revision-scoped `quality_review` state. `GET /api/watchlist/report-quality-audit/review?filename=...&pipeline=v1` reads one current target; when `pipeline` is omitted or a placeholder such as `N/A`, the report filename supplies the remaining pipeline identity. `POST /api/watchlist/report-quality-audit/review` requires the current revision, a mutation token, one decision from `approved_with_gap`, `rejected`, or `deferred`, and a non-empty note. The append-only ledger is stored in `operational.sqlite3` as `report_quality_review_events`; writes canonicalize pipeline identity from the stored/index value and fall back to the filename when that value is a placeholder. Reads apply the same identity rule, so legacy events stored with a placeholder pipeline remain addressable by their canonical filename/pipeline target. `approved_with_gap` means an operator accepted the report while retaining the metadata gap, not that any quality gate passed. The revision fingerprint uses report identity plus content/data hashes; index write time (`updated_at`) and filesystem mtime are refresh signals, not report versions. A changed report-index/artifact content fingerprint makes the old review inapplicable and returns `409` until the operator reviews the new revision. The endpoint never writes artifacts, report index rows, or rerun jobs. Historical audit items may also expose `quality_review_history`, containing the newest 20 events for the same filename/pipeline/revision in descending event order; events from an older revision are excluded.
...
`decision_queue.items` report repair actions preserve `blocks_auto_rerun` and `reason_codes` from `report_quality_repair_queue`, so API consumers can distinguish a blocked manual review from a safe rerun/refresh action without re-reading the repair queue payload. Repair items also canonicalize `pipeline_id` from the report's stored identity and fall back to the report filename when the stored value is a placeholder, so a v4 action is not displayed or routed as v1 merely because legacy metadata is incomplete.
