...
The historical quality audit renderer applies the same finite non-negative integer contract to audit, gap, provenance, rerun, review, version, artifact, snapshot, pipeline, and bounded item counts; malformed or fractional values are omitted, and invalid core scope counts show `資料需確認` instead of being floored into a report count. The core scope also requires `quality_metadata_missing_reports <= audited_reports`; a contradictory payload is not presented as a valid gap total.
