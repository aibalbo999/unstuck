...
3. **組合健檢**：持股可從追蹤股票、既有報告與常用股票清單選擇，輸入權重後按「加入／更新持股」，也可從目前持股清單移除。另可選擇 .csv 檔案，或直接貼上與編輯含 `ticker`，以及 `weight` 或 `market_value` 的內容，再按「分析目前組合」。先看每檔實際金額與前三項調整，再到資金配置、曝險、論點與調整清單查看原因。
...
- 持股選擇器只編輯 weight 類型；market_value CSV 請使用檔案匯入或文字編輯，避免把市值誤當成權重。
