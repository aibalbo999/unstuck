...
工作台背景載入 `GET /api/watchlist/current-quality-summary` 後，會把它併入同一批 latest-per-ticker/pipeline 報告的目前規則唯讀投影；它不是已保存 metadata coverage。`report_conformance_by_status` 是整體目前品質分布；`content_credibility_by_status` 與 `evidence_exit_gate_by_verdict` 是另外兩個 gate 分布，各自都以 `audited_reports` 為分母。status map 只接受 `passed|warning|blocked|unknown`，verdict map 只接受 `approved|caution|rejected|unknown`；未知 key 不會被靜默忽略，前端會放棄整份 projection。`non_passed_reports` 是至少一個 gate 非 passed 的 union count，不得大於 `audited_reports`；若超出分母，watchlist/history current-quality helper 會放棄整份 projection。畫面只展開最多 5 筆非 passed 目標；每筆目標必須有可開啟歷史檢視的非空 `filename`、明確三個 gate 狀態，且至少一個 gate 非 passed，否則整份 projection 不顯示。請用 `items_total`、`items_returned`、`items_limit`、`items_truncated` 解讀完整數字與可見範圍；若 returned 超過 limit，或 returned 小於 total 但旗標不一致，畫面會標示範圍資料需確認，不把 total 寫成完整清單。點擊只進入既有歷史核對，不會自動建立 queue、review 或 rerun。摘要讀取失敗時，既有 daily 工作台仍可使用。
