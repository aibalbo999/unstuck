...
當 evidence gate 不是 `approved` 時，content credibility 的 `non_approved_evidence_gate` warning 會附上同一份 gate 的 compact 摘要：`evidence_claim_count`、`evidence_sampled_count`、`evidence_failed_count`、`evidence_unverifiable_count` 與 `evidence_unverifiable_reason_counts`。若 current projection 已將同一 gate 重驗為 `approved`，舊的 evidence-alignment warning/blocker 會被抑制；`caution`／`rejected` 則保留。這些欄位只協助人工分流，不複製 claim 明細、不改 verdict，也不建立 review、rerun、repair 或 queue side effect。
