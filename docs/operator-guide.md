...
今日工作台的全量品質摘要會先顯示目前 scope，再分行列出缺口總數、缺 gate、模式、審核狀態、人工進度、來源、artifact 摘要與 coverage；這些是同一個 daily dashboard response 的閱讀投影，不代表新增的全庫判定。若同時有 repair queue，摘要會另顯示「修復 queue 範圍：取樣 N 份報告」；這是近期修復工作樣本，不是全量 quality audit 的分母，也不代表所有 quality 缺口都已進入 queue。手機窄版會將各項堆疊排列，target 本身仍另顯示該報告的審核狀態、結構化缺口、artifact marker 與「artifact 摘要僅供人工核對，不代表 gate 已通過」。
