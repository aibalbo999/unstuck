...
今日工作台的全量品質摘要會先顯示目前 scope，再分行列出缺口總數、缺 gate、模式、審核狀態、人工進度、來源、artifact 摘要與 coverage；這些是同一個 daily dashboard response 的閱讀投影，不代表新增的全庫判定。若同時有 repair queue，摘要會另顯示「修復 queue 範圍：取樣 N 份報告」與「品質缺口與 repair sample」交集。`complete` 時可讀 sample 內／外的精確數字；`partial` 時只表示已載入的缺口明細，未展開部分不能判定是否在 sample。近期修復工作樣本不是全量 quality audit 的分母，也不代表所有 quality 缺口都已進入 queue。手機窄版會將各項堆疊排列，target 本身仍另顯示該報告的審核狀態、結構化缺口、artifact marker 與「artifact 摘要僅供人工核對，不代表 gate 已通過」。
