...
The read-only report quality audit exposes `missing_quality_fields`, `severity`, and `action_label` on each affected item plus `items_returned` and `items_truncated` on the envelope. `severity=blocked` and `action_label=人工審核` are direct action metadata, so queue consumers do not need to infer the next step from the detail text. When `items_truncated=true`, the dashboard is showing only the returned detail rows; use the report history search for the remaining items. The historical audit view also checks `items_total/items_returned/items_limit/items_truncated` as one bounded scope; if they contradict, it labels the visible range as requiring confirmation and keeps targets available for manual review. These fields are exact audit evidence, not a reconstructed quality result, and do not enqueue a rerun or repair.
