...
- Report index, preview, compare, rerun, and HTML/Markdown download readers fail closed when an artifact is not UTF-8, not valid JSON, or not a JSON object; list/preview fallbacks remain unavailable and rerun/download paths return HTTP 400 rather than guessing or emitting replacement characters.
