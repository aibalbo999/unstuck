...
今日工作台的全量報告品質摘要也使用同一個進度算法，但它的範圍是每個 ticker/pipeline 最新一筆；完整且未截斷的 latest 缺口會以「品質稽核」人工核對 action 顯示在今日決策 queue，與 repair sample 重疊時以 repair action 為準。partial、不可用或 historical 稽核明細不進今日 queue；這些都是唯讀導引，不會自動重跑或寫入品質狀態。
