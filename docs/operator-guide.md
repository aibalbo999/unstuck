...
若「壓力位／支撐位」在同一 claim 以 `目前`、`目前處於`、`目前為` 或 `目前價位即為` 直接說明目前就是 52 週高／低點，系統才會核對對應的 canonical 高低點。canonical 數值不一致會保留 mismatch；後文另列的第二個數字、目標價、停損價與次要壓力價不會回溯借用，仍需人工確認。

「波段天花板」只有在同一 claim 明示 `52 週最高價`、`market_data` 且帶 TWD／元單位時，才會核對 `data.week_52_high`；沒有 52 週標記、只是歷史高點、目標價或停損價時，仍需人工確認，不會因數值相同而借用。
...
Prometheus `_metric_bool` 也會用同一套 explicit boolean 規則處理 queue availability、free mode 與 observability 狀態。`available`／`unavailable` 仍維持相容；像 `"healthy-ish"`、`"available-ish"` 這類未知 token 會保守視為 false，不會在 `/metrics` 製造健康或可用 gauge。
...
LLM quota handling distinguishes explicit per-key/model RPD disable from generic project/model quota exhaustion. When all keys for one model exhaust quota retries, the Worker publishes a Redis-backed model-scoped circuit with the configured TTL; later jobs fast-fail before a provider request and follow the configured fallback model. The circuit does not disable other models, and 5xx/provider-overload retries remain on their existing retry path. Redis is the shared boundary for this cross-job guard; inspect `redis-cli -u "$REDIS_URL" --scan --pattern 'stock-agent:llm:model-circuit:*'` without exposing key values.

The `LLM/API 本機觀測` surfaces are local `api_usage_events` ledger evidence: configured service count, observed calls, and observed quota/provider errors. Gemini also shows observed calls and quota/rate-limit errors by model, plus the derived local error rate, so operators can see whether failures are concentrated on one route. The route budget keeps node telemetry and provider ledger evidence separate: `provider_error_scope=recent_api_usage_events` is a bounded recent sample, not the full reset-window total. A warning means this local observation window needs attention; it is not proof of provider-wide health or remaining external quota. Use the reset time, provider response, and report-level data trust together before deciding whether to retry or rerun.

Provider SLA panels report system-level health over the selected time window, not the readiness of every stored report. A core-source critical notice means the source was unstable during that window; decide whether one report needs refresh or rerun from that report's `data_trust`, `decision_freshness`, and the daily decision dashboard queue. The panel therefore directs operators to report quality evidence and `今日工作台` instead of treating a historical provider alert as an automatic rerun instruction.

When a core source has a healthy provider in the same selected window, the ops dashboard keeps the system-level `critical` alert but adds `current_source_has_healthy_entry=true`. This records fallback coverage without silently downgrading a provider incident or claiming that every report is ready; use the report's data trust and `今日工作台` for report-level action.
...
The read-only report quality audit exposes `missing_quality_fields`, `severity`, and `action_label` on each affected item plus `items_returned` and `items_truncated` on the envelope. `severity=blocked` and `action_label=人工審核` are direct action metadata, so queue consumers do not need to infer the next step from the detail text. When `items_truncated=true`, the dashboard is showing only the returned detail rows; use the report history search for the remaining items. The historical audit view also checks `items_total/items_returned/items_limit/items_truncated` as one bounded scope; if they contradict, it labels the visible range as requiring confirmation before showing a page-number range and keeps targets available for manual review. These fields are exact audit evidence, not a reconstructed quality result, and do not enqueue a rerun or repair.
...
品質缺口摘要另會按版本顯示「目前版本缺口／歷史版本缺口／版本未判定」。target 的「目前版本（ticker/pipeline 最新）」代表這筆是目前索引列；「歷史版本（非目前最新）」只代表舊 artifact 仍有 metadata 缺口，不可直接當成目前報告已失效。歷史稽核的 coverage 與人工審核進度應先搭配版本分布解讀，再決定是否開啟目前版本或舊版報告。
...
歷史稽核回應中的 `current_quality_summary` 是另一個目前規則投影：`scope=historical_filter_current_latest` 只看相同 `q`/`pipeline` 篩選下每個 ticker/mode 的最新版本，並用自己的 `audited_reports` 分母呈現目前一致性、內容可信度與證據關卡。`non_passed_reports` 是至少一個 gate 非 passed 的 union count，不得大於 `audited_reports`；若超出分母，前端不接受這份 current-quality projection。前端只有在 `items_returned <= items_total`、等於 `items[]` 長度，且每筆 target 有非空 `filename`、三個明確 gate 狀態並至少一個 gate 非 passed 時才接受這份 bounded metadata；矛盾或不可導覽資料會被整份捨棄，不會進入摘要。它不會改寫歷史 metadata coverage，也不會把缺 gate 補成通過；看到 persisted coverage 與 current summary 不同時，代表兩個時間/語意層次不同，應分開判讀。

current-quality summary 另提供 `evidence_failed_count` 與 `evidence_unverifiable_reason_counts`：前者彙總同一批 evidence gate 的數值 mismatch，後者彙總未驗證原因；每個 target 也保留該報告的 counts。`evidence_mismatch_claims_by_freshness` 是 mismatch claim 數，`evidence_mismatch_reports_by_freshness` 是受影響報告數，兩者都按 `current`、`needs_rerun`、`unknown` 分組；target 的 `evidence_mismatch_freshness_status` 只在有真實 mismatch 時出現。`evidence_unverifiable_claims_by_freshness` 是以 gate canonical `unverifiable_count` 為準的 residual claim 數，`evidence_unverifiable_reason_counts_by_freshness` 再按本文 freshness 分組原因；target 的 `evidence_unverifiable_freshness_status` 只在有 residual 時出現。`evidence_unverifiable_reports_by_freshness` 顯示每個 freshness bucket 涉及的報告數，避免把多個 residual claim 誤當成同數量報告；若 legacy gate 沒有 reason map，畫面會標示「原因未記錄」，不猜測 reason code。畫面會用白話標籤顯示「證據數值不一致」「證據未驗證版本：資料已更新、本文需完整重跑」「研究來源非 canonical」等；這些是分流上下文，不代表 mismatch 或 unverifiable 可以忽略，也不會把 sampled claim、跨 provider 數值或任何 review/rerun/repair/queue 狀態寫回系統。沒有 canonical 欄位的衍生指標、情境目標與舊結論仍維持不可驗證，不能借用相同數值或 gate artifact 代算。

`quality_gate_action_counts` 是 current-quality summary 對每份目前報告套用 `quality_gate_repair_item` 的唯讀投影，不是 daily decision queue 的待辦數；`quality_gate_action_scope.is_daily_queue=false` 時，畫面會標示「品質處理建議（唯讀品質投影，不等同今日待辦）」。daily queue 仍依自己的近期 repair sample、完整 audit gap 與其他來源計算，兩者不可互借分母或把投影當成已排入 queue 的 action。缺少這個 optional scope 欄位的舊 payload 維持一般 action 文案；若 freshness 分布存在但格式錯誤或按 action 加總對不上總數，畫面保留可信的總 action 數，不顯示該分布。
...
開啟特定報告的 review target 時，若沒有提供模式，或模式欄位是 `N/A` 等 placeholder，系統會以檔名中的模式辨識報告；寫入與讀取 review ledger 使用同一套 identity。舊事件即使曾保存 placeholder pipeline，也仍會被歸回正確的檔名／模式／revision，不會因資料格式較舊而顯示成另一個模式的審核狀態。
...
重新整理或切換 history scope 時，頁碼會從第 1 頁開始；報告 preview 與追蹤股票快照不會被保存或沿用。切換篩選時若上一個快照仍在載入，晚到的回應也不會重新顯示在新範圍，避免把舊 ticker 的內容誤讀成目前列表的結果。
...
今日工作台的全量品質摘要會先顯示目前 scope，再分行列出缺口總數、缺 gate、模式、審核狀態、人工進度、來源、artifact 摘要與 coverage；這些是同一個 daily dashboard response 的閱讀投影，不代表新增的全庫判定。若同時有 repair queue，摘要會另顯示「修復 queue 範圍：取樣 N 份報告」與「品質缺口與 repair sample」交集，並以 `items_limit`、`items_returned`、`items_truncated` 明示目前只載入多少筆 repair target；`action_required` 是取樣內完整可處理數，不能拿可見卡片數代替。當四個 bounded 欄位都存在且彼此一致時，摘要會顯示「修復 queue：顯示 N / 共 M」；operator dashboard 與 watchlist board 共用同一個 formatter，legacy 或矛盾 metadata 不會由前端猜算，維持既有摘要文字。品質缺口主摘要也會一起核對 `items_total`、`items_returned`、`items_limit`、`items_truncated`；矛盾時保留 target，但標示範圍資料需確認，不把可見列當成完整缺口清單。`complete` 時可讀 sample 內／外的精確數字；`partial` 時只表示已載入的缺口明細，未展開部分不能判定是否在 sample。近期修復工作樣本不是全量 quality audit 的分母，也不代表所有 quality 缺口都已進入 queue。手機窄版會將各項堆疊排列，target 本身仍另顯示該報告的審核狀態、結構化缺口、artifact marker 與「artifact 摘要僅供人工核對，不代表 gate 已通過」。
...
分析新鮮度與目前品質摘要也只接受有限的非負整數分布；若 aggregate 或明細範圍出現小數、無限大或格式錯誤，摘要會整體省略，避免把四捨五入後的假完整度當成可信範圍。

watchlist 的 repair queue 取樣範圍也只顯示有限的非負整數；小數或格式錯誤會省略取樣數，避免把被截斷的數字誤當成精確範圍。
...
歷史版本品質稽核的報告、缺口、審核、版本、artifact、snapshot 與分頁計數也只接受有限非負整數；fractional 或 malformed 值不會被截斷成假數字，且缺口數不得超過稽核總數，若有已驗證快照分母也不得超過該分母。若有完整 metadata 計數，還必須和缺口數加總等於已驗證快照數；若三種 snapshot scope 計數都存在，verified、invalid、未驗證三者也必須加總等於稽核總數；若有 `items_total`，也必須等於主缺口數，若有 `items_returned`，也必須等於實際返回的明細筆數；若 total、returned、offset 都存在，返回視窗也不得超出 total。核心範圍不完整或彼此矛盾時會顯示「資料需確認」。
...
每筆人工核對區也會顯示目前報告的版本識別碼：畫面用短版方便掃讀，滑過或使用輔助技術時可取得完整識別碼。核對前請確認它與正在查看的報告版本一致；這個提示不能取代 server 的 revision 檢查。

歷史摘要的「人工審核進度：已決策／總缺口」只計算目前 response 範圍內的缺 metadata 報告；切換 pipeline 或 review status 後，分母也會跟著範圍改變，不是全庫永久累計值。

今日工作台的全量報告品質摘要也使用同一個進度算法，但它的範圍是每個 ticker/pipeline 最新一筆；完整且未截斷的 latest 缺口會以「品質稽核」人工核對 action 顯示在今日決策 queue，與 repair sample 重疊時以 repair action 為準。帶 filename 的品質稽核 queue item、操作員工作台 action、notification message/outbox 會共用 `operator_action=quality-audit-review`、`operator_action_label=前往人工核對`、`target_panel=history-quality-audit`、`target_tab=analysis`，並以 filename/pipeline 直接開啟同一份 targeted historical audit；明確自訂 metadata 仍優先。帶 filename 的一般 report repair manual review 也會在 queue、notification message/outbox 帶出 `view-report`、上游 `action_label` 與 `active-jobs-panel/ops`，直接開啟報告；沒有 filename 的 quality action 不套用 targeted history。partial、不可用或 historical 稽核明細不進今日 queue；這些都是唯讀導引，不會自動核准、重跑或寫入品質狀態。
...
模式 D 的交易計畫若在目標價或停損欄位放入多個非區間情境價格，品質檢查會顯示「需人工核對」警示，並在 check details 保留 `target_price_candidates` 或 `stop_loss_candidates`；這不是自動否定 Neutral。百分比調整（例如 `回檔逾 10%`、`-4.5％`）不是價格，不會進入候選清單；明示 `28.2x PE band`、`18x PE band` 的估值倍數也不是價格，但沒有 PE/本益比/估值語境的 `x/倍` 不會一律刪除。同一 issue 若有 persisted 舊診斷，current projection 的 details 會優先呈現目前解析結果。明確由區間分隔符連接的兩端價格（包含 `121.0 TWD 至 130.5 TWD`、`419.15 TWD 至 52 週高點 460.0 TWD` 這類格式）會視為單一區間；括號內明示為高低點、壓力或支撐的參考價（例如 `46.6 TWD（挑戰 52 週高點 46.63 TWD）`）也不會被當成第二個情境。人工核對時應回到報告原文，確認每個獨立情境的觸發條件、目標與停損是否成組。
...
報告列表的 `data_trust_status` 是從同一份已正規化的 `data_trust` 投影而來，不採用 legacy index duplicate 欄位的原始字串；因此列表不會同時顯示兩個不同的資料可信度狀態。若 trust JSON 無法讀取，應以 `unknown` 進行人工確認，不能以 duplicate 欄位猜測為 fresh 或 stale。

報告列表也會用明確 boolean 規則讀取 legacy `analysis_text_stale`：字串 `false` 不會因 JavaScript/Python truthiness 被當成 stale。這個旗標會同時影響結論 freshness 與 recommendation calibration，因此 malformed 值應保守維持 false，不可把資料格式問題誇大成「本文需重跑」。
...
畫面會把 `snapshot_field_unavailable` 顯示成「快照欄位不可用」；這表示來源沒有可核驗值，不表示數字是零，也不表示 parser 漏掉資料。

熊市／基本／牛市情境標籤（包含 `熊/基/牛情境`），以及表格第一欄明示 `熊市`、`基本` 或 `牛市` 的情境目標，若沒有 canonical 情境 scalar，會顯示 `scenario_target_not_canonical` 並維持 `unverifiable`。不要用 `content_credibility`、DCF intrinsic value、現價或其他目標價補值；只有同一路徑的 canonical 情境欄位才依一般 matched/mismatch 規則核對。

5 年成長情境表若第一欄是 `保守`、`悲觀`、`中性`、`基準` 或 `樂觀`，且附近文字是情境預測、年營收或 CAGR（或數值單位是 `億`），沒有 canonical 預測路徑時會顯示 `analysis_metadata_not_evidence` 並維持 `unverifiable`。不要把現況營收、分析師目標價或另一個情境數字借來當成預測證據。

`心理關卡`、`第二支撐`、`關鍵支撐區`、`近期支撐`、`支撐位` 這類敘事技術價位若沒有 canonical 技術價位／`risk_price` scalar，會顯示 `technical_level_not_canonical` 並維持 `unverifiable`。不要借用現價、target candidates 或附近歷史數字；若有 `risk_price`，才依一般 matched/mismatch 規則核對。句內明示 `Agent 3 評分` 的長敘述則歸為 `analysis_metadata_not_evidence`，不因 label 前面有敘述文字而漏掉分析 metadata 邊界。
...
「盤中速報」也屬於新聞來源邊界；即使同一數字出現在 target-price candidates、現價或 `risk_price`，仍顯示 `news_source_not_canonical`，不把盤中提及價升級為 canonical evidence。
...
籌碼中文別名要看上下文：`近 5 個交易日淨買超 (千股)` 只核對 `data.institutional_trading.last_5_trading_days_net_buy_thousand_shares`；`法人合計買超` 只有前文明示五日區間時才核對同一欄位。`小戶 (持股 < 50 張) 比例` 只核對 TDCC retail-holder 欄位，`權益負債比率` 只核對 `data.debt_to_equity`。獨立的 `S&P 500` 百分比只有明示 `1d` 才核對 SPY 的 `change_1d_pct`；指數目標點位不能借用日變動欄位。即使相鄰 total、major-holder、current-ratio 或其他 symbol 恰好同值，也要保留原本的 canonical path 邊界。
短 label `Trust` 只有在同一行明示 `Net Buy (30 days)`，並依序列出 `Foreign`、`Dealer`、`Trust` 的法人分類時，才核對 `data.institutional_trading.net_buy_thousand_shares_by_category.investment_trust`。孤立 `Trust`、缺少分類上下文或同值的 Foreign/Dealer 欄位，都要保留不可驗證，不得自行推定為投信。
...
compact `最終投資建議` row 的 `避免；3個月`、`6個月`、`12個月`，以及 `NT$209.0；6個月` 這類幣別前綴 horizon label，若來自沒有 persisted `rerun_context.parsed`／`structured_outputs` 的 legacy snapshot，且沒有 canonical target path，請以 `legacy_conclusion_without_snapshot_path` 分流；這只表示「舊結論缺少可重建上下文」，仍須人工確認。若 snapshot 有 parsed 或 structured context，仍用 `missing_semantic_path`；一般營收或其他非投資建議的月份 label 不套用 legacy reason。不要把 `content_credibility`、`report_conformance`、`analyst_target` 或另一個 horizon 的數值當成來源證明。
...
數字抽取會保留千分位整數，即使數值後面直接接下一句，例如 `1,177,000. Borrowed short sale today: 21,000.` 不會被截成 `1,177`。`券資比` 與 `潛在下行空間` 這類衍生指標，若 snapshot 沒有 canonical ratio／`downside_pct` scalar，會維持 `unverifiable`／`derived_metric_not_canonical`；`防軋空停損點`、`價格停損條件` 若沒有 canonical `risk_price`／stop-loss scalar，則分流為 `risk_control_not_canonical`。不要把輸入欄位當成比例、百分比或風險控制價格的來源，也不要自行重算成已核驗證據；若 snapshot 明確提供 canonical field，才依一般 matched/mismatch 規則判定。

Evidence parser 也會排除模型 fallback／error 後緊接的 HTTP 4xx/5xx 狀態碼，以及 `30-day` 這類期間文字；這些不是報告數值證據。真正的 `target price: 429 TWD` 仍會抽查，不會因數字剛好像錯誤碼而被刪除。
...
月份高點／低點若省略年份，只有在 claim 明寫 `6 月`／`6 月份` 等月份語意，且 snapshot 該月份只有一個年份時，才會核對 `data.price_history[month=YYYY-MM].high|low`；年份明寫時使用指定年份。跨年份、新聞／催化劑，或數值不是月份極值前緊鄰的 claim，請保留 `unverifiable`／`mismatch`，不要依同值或鄰近日期推定。
...
在報告列表/歷史 row 中，`evidence_exit_gate` 可能是 current read-only projection；請同時查看 `evidence_exit_gate_projection.persisted_verdict` 判斷原始 snapshot 的 gate。`content_credibility` 也會使用同一份 current evidence projection，避免舊 `approved` 把內容可信度誤顯示為通過；若 legacy snapshot 缺少完整 parsed context，系統只刷新 evidence-confidence check，不把 partial result 當成完整內容重算，v4 交易計畫仍會走既有 fallback。若 index recommendation 足以建立 deterministic aliases，row 會標示 `content_credibility_projection.source=snapshot.recommendation_context`；這仍是可追溯的唯讀 projection，不代表 persisted quality metadata 已補齊。`report_conformance` 同樣可能是 current composite projection，請查看 `report_conformance_projection.persisted_status`；它只反映目前 evidence/content gate，不會自動修改原始 conformance、review、quality coverage 或 queue。直接開啟報告 HTML、下載 HTML 或下載 Markdown 時，閱讀提示與 HTML execution summary 的 Evidence gate、Content credibility、Report conformance 三欄都會在 response time 使用同一份 current quality gate，所有已知狀態（passed、warning、blocked、pending）都會對齊，避免不同格式各自顯示不同結果；Final audit 與 Report lint 若沒有 current projection 仍保留 persisted 值。這些都是 view-time projection，不會回寫 HTML/Markdown/data snapshot。`/api/report/{filename}/download/data` 仍是原始 snapshot，projection 不代表資料已回寫，也不會改變品質稽核的 metadata coverage 或自動重跑判定。
...
若同一 response 有 `decision_freshness_items`，工作台只顯示有限的待重跑報告導覽 sample；請以 `items_total` 判讀全量數字，以 `items_returned/items_limit/items_truncated` 判讀畫面實際展開範圍。若 returned 超過 limit，或 returned 小於 total 但截斷旗標缺失／矛盾，畫面仍保留 target 入口，但會標示範圍資料需確認，不把 total 寫成完整清單。點擊 target 只會開啟既有歷史報告檢視，不會自動加入 queue 或觸發重跑。

工作台背景載入 `GET /api/watchlist/current-quality-summary` 後，會把它併入同一批 latest-per-ticker/pipeline 報告的目前規則唯讀投影；它不是已保存 metadata coverage。backend 會透過 pagination 載入完整 index；若頁面缺失、格式錯誤、宣告總數不一致或中途提前變短，會回報 unavailable，不把較小的實際載入列誤當成完整 scope。`report_conformance_by_status` 是整體目前品質分布；`content_credibility_by_status` 與 `evidence_exit_gate_by_verdict` 是另外兩個 gate 分布，各自都以 `audited_reports` 為分母。status map 只接受 `passed|warning|blocked|unknown`，verdict map 只接受 `approved|caution|rejected|unknown`；未知 key 不會被靜默忽略，前端會放棄整份 projection。`non_passed_reports` 是至少一個 gate 非 passed 的 union count，不得大於 `audited_reports`；若超出分母，watchlist/history current-quality helper 會放棄整份 projection。畫面只展開最多 5 筆非 passed 目標；每筆目標必須有可開啟歷史檢視的非空 `filename`、明確三個 gate 狀態，且至少一個 gate 非 passed，否則整份 projection 不顯示。請用 `items_total`、`items_returned`、`items_limit`、`items_truncated` 解讀完整數字與可見範圍；若 returned 超過 limit，或 returned 小於 total 但旗標不一致，畫面會標示範圍資料需確認，不把 total 寫成完整清單。current-quality 的 bounded cache 以 projection scope/filter 加上相同 latest index-row fingerprint（`updated_at`、檔案 mtime、內容/資料 hash）分開保存；索引列變更會立即淘汰舊 projection，indexed 與 historical-filter 不會互相命中，fingerprint 無法讀取時則不重用 cache。點擊只進入既有歷史核對，不會自動建立 queue、review 或 rerun。摘要讀取失敗時，既有 daily 工作台仍可使用。
...
當 evidence gate 不是 `approved` 時，content credibility 的 `non_approved_evidence_gate` warning 會附上同一份 gate 的 compact 摘要：`evidence_claim_count`、`evidence_sampled_count`、`evidence_verified_count`、`evidence_failed_count`、`evidence_unverifiable_count` 與 `evidence_unverifiable_reason_counts`。其中 `evidence_verified_count` 是抽樣 claim 中 status 為 `verified` 的數量，方便操作員分辨已核對覆蓋率與剩餘人工確認量。若 current projection 已將同一 gate 重驗為 `approved`，舊的 evidence-alignment warning/blocker 會被抑制；`caution`／`rejected` 則保留。這些欄位只協助人工分流，不複製 claim 明細、不改 verdict，也不建立 review、rerun、repair 或 queue side effect。
...
- Report index, preview, compare, quality-audit evidence, rerun, and HTML/Markdown download readers fail closed when an artifact is not UTF-8, not valid JSON, or not a JSON object; list/preview fallbacks remain unavailable, quality-audit marker/context fields remain unavailable, and rerun/download paths return HTTP 400 rather than guessing or emitting replacement characters.
...
- Report refresh response `decision_freshness` reads the same refreshed snapshot that was just persisted, so storage implementations without a filesystem path cannot downgrade a valid `needs_rerun` result to `unknown`.
...
When no work exists, `decision_queue.items` may still contain the `monitor` compatibility item, but `summary.displayed_count=0` and canonical `summary.secondary_count=0`; the placeholder is not a quick action, pending item, or notification. The top-level `decision_queue.secondary_count` is retained as a legacy alias, and consumers prefer the summary value when both are present.
...
`fix_notification_delivery` is queue-only: its CTA is `查看通知通道`, its target is `maintenance-panel` / `ops`, and `suppress_notification=true` intentionally prevents a broken notification channel from notifying through itself. The explicit queue target keeps the API and operator UI on the same maintenance surface.
