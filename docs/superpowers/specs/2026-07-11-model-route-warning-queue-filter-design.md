...
「今日待處理」不再提出純效能型模型路由警示，避免正常但較慢的模型呼叫占用操作者待辦；重試風暴仍保留，因為它代表路由可靠度已受影響。

## 行為邊界

- 從 `daily_decision_queue` 排除 `slow_route`。
- 保留 `retry_storm`，因為它代表重試失敗型路由風險，仍需要人工注意。
...
- 驗證 `slow_route` 不會出現在 decision queue。
- 驗證 `retry_storm` 與 `quality_gate_failures` 仍會出現，且來源與 action type 維持 `model_route_budget`／`model_route_warning`。
