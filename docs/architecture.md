...
歷史品質缺口的人工決策另走 `report_quality_review_store`：它使用 canonical `operational.sqlite3` 的 append-only `report_quality_review_events`，以 report-index/artifact fingerprint 綁定 `report_quality_revision`。`approved_with_gap`、`rejected`、`deferred` 只記錄操作人員對目前 evidence 的決策與理由；revision 改變時舊決策不會套用。mutation endpoint 不回寫 artifact、quality gate、report index，也不 enqueue rerun。
