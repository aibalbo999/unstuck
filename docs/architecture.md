...
歷史頁勾選「顯示舊版報告」後，前端以目前搜尋與 pipeline 篩選呼叫同一 historical endpoint；`history_quality_audit` 只負責摘要與 report preview target，並在背景載入，不阻塞既有歷史報告列表。摘要明示 `verified_snapshot_reports` 分母，並在 invalid/unverified snapshot 存在時顯示排除警示；模組以 load generation 實作 latest-request-wins，避免快速切換篩選時舊回應覆蓋新範圍。
