...
Daily report quality uses the full latest-per-ticker/pipeline audit for coverage, while the repair queue intentionally uses a recent sampled report set for next-action ranking. These scopes remain separate; the UI surfaces `repair_queue.summary.sampled_reports` as a boundary note instead of implying that every audit gap is already a repair action.
