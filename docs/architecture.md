...
Daily report quality uses the full latest-per-ticker/pipeline audit for coverage, while the repair queue intentionally uses a recent sampled report set for next-action ranking. These scopes remain separate; the UI surfaces `repair_queue.summary.sampled_reports` as a boundary note and exposes `report_quality_audit.repair_sample_overlap` by filename/pipeline. The dashboard's backward-compatible `summary.reports_needing_rerun` and `summary.report_repairs_required` counts are also sample-scoped, with `summary.report_scope` explicitly identifying `daily_report_sample` and its sample size. Only a complete audit item set can provide an exact sample-outside count; a paginated set is marked `partial` without guessing at unexpanded gaps. A complete, untruncated latest audit can additionally project its non-overlapping gap items into `decision_queue` as `source=report_quality_audit` manual-review actions; repair-sample items remain authoritative on overlap, while partial, unavailable, and historical audit scopes stay excluded. This presentation path is read-only and does not enqueue reruns or mutate review, artifact, index, or repair state.
...
The same current-quality summary keeps evidence mismatches traceable to conclusion freshness: `evidence_mismatch_claims_by_freshness` counts failed sampled claims and `evidence_mismatch_reports_by_freshness` counts affected reports under `current`, `needs_rerun`, or `unknown`. Each bounded target with a real mismatch carries `evidence_mismatch_freshness_status`. The shared browser helper renders this context without changing evidence verdicts or creating any side effect.
...
前端的 `report_quality_evidence_helpers.js` 是 preview、history audit 與 daily watchlist target 的共同 evidence context；它只負責欄位標籤、provenance、artifact marker 與 limitation 的一致表達。對有 structured gap 的 target，它另外輸出短版 artifact warning，供 renderer 以可見小字與 aria-label 呈現；preview badge 的 `title`、aria-label 與 `data-quality-evidence-detail` 優先使用同一個 `context(report).detail`，但保留 badge 專用的 compact inline 容器，不直接重用 target `<small>` HTML。這些都是 presentation projection，不是 gate verdict。preview 的 structured-gap badge 透過既有 `StockAgentOpenHistoricalQualityAudit` 以 filename/pipeline 導航到同一個 read-only audit scope，不能繞過 review mutation boundary。

歷史 audit renderer 會把目前套用的 `review_status`/`missing_field` scope 置於摘要最前方，並將既有 envelope 的欄位、模式、review、進度與 provenance 分成獨立可掃讀項；窄版只改 CSS 排版，不改 API 數字或 coverage 分母。這是 UI reading order，不能被當成新的 backend scope 或品質判定。
...
同一 envelope 的 `quality_metadata_missing_by_provenance` 會把缺 metadata 的報告分成 `before_refresh`、`after_refresh` 與 `no_refresh_provenance`；`before_refresh` 只在 pre-refresh trace 已列出目前缺口時成立，`after_refresh` 只表示有 `refreshed_from_report` attribution，不能推論刷新造成缺失；後者只表示沒有可用分類，也不能推論「從未刷新」。audit item 會保留 provenance、刷新來源檔名與 `snapshot_refreshed_at`，但不從 HTML/Markdown 重建 gate payload；前端以「刷新前已有缺口」、「有刷新歸因」或「未標記刷新來源」呈現。
...
同一 envelope 的 `quality_metadata_by_pipeline` 會保留每個模式的 verified 分母、coverage、execution strategy 與 context readiness；daily/history 摘要會把有缺口的模式與上下文準備度分開顯示，再把模式轉成唯讀快速篩選鈕，沿用 history workspace 的 `pipeline` filter、重設頁碼與 preview 後重新載入，不新增另一套查詢或 action。需要深入時仍可使用 `q`/`pipeline` filtered historical audit。
...
Quality audit target 另外分開呈現 `missing_quality_fields` 與 `artifact_quality_summary.fields`：前者是 verified snapshot 缺少的結構化 metadata，後者是 artifact 中可供人工核對的 marker evidence。前端不得用後者補寫或推導 gate 狀態；兩層在 daily/history renderer 分別標為「結構化缺口」與「artifact 摘要可查」。
