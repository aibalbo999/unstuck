...
Daily report quality uses the full latest-per-ticker/pipeline audit for coverage, while the repair queue intentionally uses a recent sampled report set for next-action ranking. These scopes remain separate; the UI surfaces `repair_queue.summary.sampled_reports` as a boundary note and exposes `report_quality_audit.repair_sample_overlap` by filename/pipeline. Only a complete audit item set can provide an exact sample-outside count; a paginated set is marked `partial` without guessing at unexpanded gaps. A complete, untruncated latest audit can additionally project its non-overlapping gap items into `decision_queue` as `source=report_quality_audit` manual-review actions; repair-sample items remain authoritative on overlap, while partial, unavailable, and historical audit scopes stay excluded. This presentation path is read-only and does not enqueue reruns or mutate review, artifact, index, or repair state.
