...
Daily report quality uses the full latest-per-ticker/pipeline audit for coverage, while the repair queue intentionally uses a recent sampled report set for next-action ranking. These scopes remain separate; the UI surfaces `repair_queue.summary.sampled_reports` as a boundary note and exposes `report_quality_audit.repair_sample_overlap` by filename/pipeline. Only a complete audit item set can provide an exact sample-outside count; a paginated set is marked `partial` without guessing at unexpanded gaps. This is read-only scope calibration and does not turn every audit gap into a repair action.
