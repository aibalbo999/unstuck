...
1. `CUTOFF_SESSION` 必須由已 pin 的 machine-readable schedule 選出最早未完成列，不以當天日期猜測；`RUN_ID` 由 planner 依相同 cutoff、實際執行時間及現有最大 revision 產生。若操作環境另給 `OOS_CUTOFF_SESSION`／`OOS_RUN_ID`，兩者都必須與 planner 結果完全相同。
...
MATURITY_FIELDS="$(
  printf '%s' "$MATURITY_PLAN" | "$PROJECT_PYTHON" -c \
    'import json, sys; p = json.load(sys.stdin); print("{}:{}:{}:{}".format(p["status"], str(p["should_capture"]).lower(), p.get("cutoff_session", ""), p.get("recommended_run_id", "")))'
)" || exit 1
MATURITY_STATUS="${MATURITY_FIELDS%%:*}"
MATURITY_REST="${MATURITY_FIELDS#*:}"
MATURITY_SHOULD_CAPTURE="${MATURITY_REST%%:*}"
MATURITY_SELECTION="${MATURITY_REST#*:}"
PLANNED_CUTOFF_SESSION="${MATURITY_SELECTION%%:*}"
PLANNED_RUN_ID="${MATURITY_SELECTION#*:}"
...
if [[ -z "$CUTOFF_SESSION" || -z "$PLANNED_RUN_ID" ]]; then
  echo "maturity planner omitted the cutoff or run ID" >&2
  exit 1
fi
...
if [[ -n "$REQUESTED_RUN_ID" && "$REQUESTED_RUN_ID" != "$PLANNED_RUN_ID" ]]; then
  echo "requested run ID does not match the pinned maturity plan" >&2
  exit 1
fi
