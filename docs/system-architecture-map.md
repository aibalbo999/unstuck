...
- `evidence_exit_gate._path_markers_for_claim()` 對 `Borrowed Short Return Today`／`Today's borrowed short return` 與同一 borrowed-short-sale claim 中的 compact `return` 建立 `chip_data.twse_margin_short_sales.borrowed_short_return_today` marker；不回退到 `borrowed_short_sale_today`，`vs Sale Today` 仍無專用語意時維持 `unverifiable`。
