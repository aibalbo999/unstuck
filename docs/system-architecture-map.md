...
- `daily_decision_queue_summary.queue_response()` 保留 `monitor` 佔位項以維持 UI 相容，但 `summary.displayed_count` 與 `secondary_count` 只計真正 actionable items；無待辦時 `total_actionable=0`、`displayed_count=0`、`secondary_count=0`，避免把健康狀態佔位誤讀成工作。
...
- `report_current_quality_summary.quality_gate_action_counts_by_freshness` 以同一 latest current scope 將品質處理建議分成 `current`、`needs_rerun`、`unknown`；它與總數欄位使用同一份逐報告 action projection，前端只有收到 optional map 才顯示「按資料新鮮度」分布。這是 read-only diagnostics，不改 action picker、daily queue、review 或 rerun state；legacy response 缺欄位時保留總數文案。
