...
- `evidence_exit_gate._check_claim()` 對 `情緒評分` 等情緒／過熱分數，在沒有 dedicated canonical score field 時輸出 `analysis_metadata_not_evidence`；`social_sentiment` freshness metadata 不作分數 evidence，也不借用其他同值數字。
