...
- `report_quality_audit.items[]` 必須保留 repair item 的 `detail`、`reason_codes` 與 `missing_quality_fields`，讓人工核對能看到精確缺少的 gate，避免 serializer 只留下 title 而切斷品質判定證據鏈；不得從 HTML/Markdown 重建 gate payload。
