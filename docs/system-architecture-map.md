...
- `evidence_exit_gate._check_claim()` 對 `券資比`／margin-short ratio 在只有融資／融券組成值、沒有 canonical ratio scalar 時輸出 `derived_metric_not_canonical`；仍維持 `unverifiable`，不自行推導、不借用 balance path，也不改 snapshot、artifact、index、review、rerun 或 queue。shared quality evidence helper 將此 reason 顯示為「衍生指標沒有 canonical 欄位」。
