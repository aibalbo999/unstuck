...
- daily dashboard 的 `repair_queue.summary.sampled_reports` 是近期 repair action sample，與全量 `report_quality_audit` coverage 分開；watchlist 只顯示 scope boundary，不把全量 quality gap 自動加入 queue。
