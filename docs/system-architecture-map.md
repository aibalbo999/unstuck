...
- `content_credibility_evidence_confidence` 對非 `approved` 的 evidence gate warning 只帶入同一 gate 的 compact count/reason summary，包含 claim、sample、verified、failed、unverifiable 與 `unverifiable_reason_counts`；`verified` 是抽樣中已核對的 claim 數量。current `confidence_evidence_alignment=passed` 且 `evidence_verdict=approved` 時，projection merge 會抑制同一批 stale evidence-alignment issue，其他 `caution`／`rejected` 仍保留。不帶 sampled claim raw text，不改 evidence verdict、content status、snapshot、artifact、review、rerun 或 queue。
