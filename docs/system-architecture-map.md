...
- `content_credibility_inputs.first_price()` 先移除明確日曆日期與週期數字（包含 `1-2週`、`1至2週`、`1 to 2 weeks` 這類期間範圍），再呼叫既有價格 parser，避免交易計畫的日期／週期 token 污染目標價或停損價；這是 read-only input normalization，不改 snapshot、index、artifact、gate 或 queue。
