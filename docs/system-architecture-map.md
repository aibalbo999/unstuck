...
- `content_credibility_evidence_matrix` 對 `最終投資建議` 不只比對 claim 名稱，還要求 row status 為 `success`、`skipped_fresh_cache` 或 `degraded_enrichment` 且有非空 basis；否則產生 `unusable_final_recommendation_evidence` warning，數字抽查仍由 `evidence_exit_gate` 負責。
