...
- historical quality audit 的 `version_status` filter 先縮小目前/歷史/未判定版本範圍，再套用 review/missing-field gap filters；只選版本時保留完整報告作為 coverage 分母，回應以 `report_version_status_filter` 記錄範圍。這是 GET-only navigation，不改 daily queue、artifact、review ledger、rerun 或 report index。
...
- `content_credibility_inputs.first_price()` 先移除明確日曆日期與週期數字，再呼叫既有價格 parser，避免交易計畫的日期／週期 token 污染目標價或停損價；這是 read-only input normalization，不改 snapshot、index、artifact、gate 或 queue。
- `price_parser.extract_target_price_numbers()` 對明確可判定的非價格 `time-to`／reached-queue 語句與字串開頭的直接目標價採 conservative fast path；商品／估值／修正幅度及含前置上下文的語句仍使用完整 fallback，避免效能優化改變價格語意。
- `report_quality_audit.items[]` 必須保留 repair item 的 `detail`、`reason_codes` 與 `missing_quality_fields`，indexed audit 可附 `artifact_quality_summary` 標示 Markdown/HTML 的可見 gate 摘要 marker，讓人工核對知道是否有 artifact evidence 可查；這個欄位不得重建 gate payload，也不得被當成通過證據。audit envelope 另保留 `items_returned`/`items_truncated`，讓人工核對能知道明細是否被限制；不得把截斷明細當成全量結果。
