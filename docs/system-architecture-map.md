...
Agent 有效設定的唯讀入口是 `/api/observability/agent-settings`（`agent_effective_settings.py`）；worker event／報告憑據以 `effective_settings_sha256` 核對實際 process。無損候選 prompt、估值預先計算值契約及獨立 key admission deadline 見 [2026-09-24 模型呼叫修正](model-call-optimization-2026-09-24.md)。

`runtime_probe_executor.py` 為 `/readyz` 與 Agent 設定查詢各提供一個專用 worker，與儀表板的共用 executor 分離；每路只允許一個進行中工作，逾時／取消保留占位直到工作完成，由 router lifespan 關閉。忙碌或逾時回傳 503，不以快取假裝查驗成功。
...
- report-facing artifact readers 對非 UTF-8、無法解析的 JSON 與非 object JSON root 必須 fail closed；列表/preview/compare 與 quality-audit marker/context 只能回傳 unavailable/fallback，rerun 與 HTML/Markdown download 必須回傳明確 400，不得以 replacement character 或其他報告的值補出可讀內容。
...
- `GET /api/watchlist/current-quality-summary` 以 ordinary report-history 的 current-rule projection 統計目前 `report_conformance`、`content_credibility` 與 `evidence_exit_gate`，三組分布各自保留 `audited_reports` 分母；backend 透過共用 pagination collector 載入完整 index，若頁面缺失、格式錯誤、total 矛盾或非最後頁提前變短，就回傳 unavailable 而不發布 undercount scope。`non_passed_reports` 是至少一個 gate 非 passed 的 union count，必須不大於 `audited_reports`，watchlist/history current-quality helper 遇到超出分母或 target shape 不完整的 payload 時 fail closed。watchlist 在快速 daily response 後背景載入，bounded cache 以 projection scope/filter 加同一份 latest index-row fingerprint（`updated_at`、檔案 mtime、內容/資料 hash）分開失效；indexed 與 historical-filter 不共用 payload，fingerprint 無法讀取時不重用舊 projection，只回傳最多 5 筆帶非空 filename、明確三個 gate 狀態且至少一個 gate 非 passed 的 navigation target，不把警示轉成 queue、review 或 rerun side effect。
- report-history pagination 的 `collection_is_complete()` 對 `pagination.complete` 沿用 shared `safe_bool`；legacy `"true"` 保留 complete、`"false"` 保留 partial，缺值維持既有 complete-by-default，unknown token fail closed，避免 current-quality/history audit 把不完整 collection 當成全量 evidence。
- 報告預覽的瀏覽器品質動作與閱讀邊界沿用後端品質 gate 的 known-state allowlist：`report_conformance`／`content_credibility` 只接受 `passed|warning|blocked|failed|rejected`，`evidence_exit_gate` 只接受 `approved|caution|rejected`；非空但未知的值一律視為未記錄，不能略過人工核對或形成 passed boundary，已知值比對不分大小寫。`snapshot_integrity=unverified` 仍保留 warning，不能因品質 gate record helper 不適用 snapshot 而被當成沒有 snapshot。
...
- `report_index.query_report_metadata()` 保留 recommendation、data-trust 與 search 的 SQL 篩選，但 pipeline 篩選與 `latest_per_ticker_pipeline` 分組在 row scope 上套用 shared `resolve_report_pipeline_id()`；legacy `N/A` + v4 filename 不會被漏出 v4 查詢或拆成另一個 latest group。total 與 pagination 也以 canonical pipeline scope 計算，且不回寫 index、snapshot 或 artifact。
- `report_refresh_service.refresh_report_data_snapshot()` 在資料刷新 persistence boundary 先以 filename + previous snapshot resolve canonical pipeline，再把同一值傳入 refresh context、provisional snapshot 與 final snapshot；legacy v4 `pipeline=N/A` 不會因只刷新資料而污染新 snapshot 的模式身份，且既有 `needs_rerun` 仍獨立判定本文是否需要完整重跑。
...
- `data_freshness`、`data_fetch.workflow`、`data_fetch.service`、provider execution、enrichment provider/merge 與 exception audit 也沿用 shared `safe_bool` 傳遞 `_cache_hit`；legacy cache token 不得在 freshness、FetchResult 或 source audit 間改變語意。
- `model_route_budget`、`job_store_telemetry` 與 `job_ops_dashboard_metrics` 對 `cache_hit`／`quality_gate_pass` 沿用 shared `safe_bool`；`None` quality gate 保留未提供，legacy false token 不得扭曲 cache rate、billable token 或品質失敗統計。
- `job_ops_dashboard_metrics.prompt_budget_summary()` 對 telemetry `cache_hit` 也沿用 shared `safe_bool`，再由 `api_observability_service` 暴露到 `/api/observability/dashboard`；legacy `"false"` 不得虛增 prompt-budget cache hit 或 estimated cached input tokens，並與 node telemetry／model-route budget 保持同一統計語意。
- `runtime_health.build_readiness_payload()` 對 storage-check `success` 與 queue snapshot `available` 也沿用 shared `safe_bool`；legacy `"false"` 必須 fail closed 為 `not_ready`，避免 `/readyz` 透過 Python 字串 truthiness 接受不可用的工作依賴，details 仍只作唯讀診斷。
- `watchlist_scheduler._run_daily_market_screener()` 對 daily screener result 的 `success` 也沿用 shared `safe_bool`，只有明確成功才寫入 run-date marker；legacy `"false"` 不得把 failed scan 標成已完成，避免抑制後續 retry。
- `watchlist_service.monitor_watchlist_triggers()` 在 event persistence 後、enqueue 前對 `matched` 也沿用 shared `safe_bool`；event audit 與 downstream job gate 必須保持同一語意，legacy `"false"` 只記錄未命中，不得建立分析任務。
- `provider_impact` 與 `queue_dashboard_payload` 的 recovery/availability flags 也沿用 shared `safe_bool`；未知 token fail closed，不得將核心 provider 誤標健康或 queue 誤標可用。
- `recommendation_calibration` 對 `analysis_text_stale` 與 `data_trust.status` 也沿用 shared `safe_bool`／`normalize_freshness_status()`；legacy `"false"` 不得阻擋 fresh-data recommendation upgrade，`" FRESH "` 不得因格式漂移被降成 watch，calibration audit 需保留 normalized values。
- `reporting.evidence_exit_gate_projection` 與 `report_compare_service` 對 `decision_freshness.requires_rerun` 也沿用 shared `safe_bool`；legacy `"false"` 不得建立 stale evidence context 或 compare warning，明確 true token 才能進入 read-only operator notice。
- `reporting.content_credibility_evidence_confidence` 對 `freshness_context.status`／`requires_rerun` 也沿用 `normalize_freshness_status()`／`safe_bool`；legacy `"true"` 必須保留為 canonical stale context，status 格式漂移不得讓 evidence-confidence warning 遺漏 freshness 證據，也不得帶出 sampled claim raw text。
- `provider_sla_dashboard_payload` 與 `api_observability_service` 對 `current_source_has_healthy_entry` 也沿用 shared `safe_bool`；legacy `"true"` 必須保留 core source health/covered evidence，unknown token 不得增加核心健康覆蓋統計。
- `daily_decision_dashboard` 的 quality-audit queue item 與 repair sample overlap projection 對 `items_truncated` 也沿用 shared `safe_bool`；legacy `"true"` 必須維持 partial audit、阻止不完整 item 進入 repair navigation，不得從 items count 猜測完整 scope。
- `report_quality_audit_rows.snapshot_integrity()` 對 verifier `valid` 也沿用 shared `safe_bool`，並讓 `status` 與 `valid` 共用同一 canonical 判定；legacy `"false"` 不得把 invalid snapshot 投影成 verified，hash verification 本身仍由既有 verifier 負責。
- report index row、歷史 artifact reading notice、reading notice 與 report repair queue 對 recorded snapshot integrity `valid` 也沿用 shared `safe_bool`；legacy `"false"` 即使與 `status=verified` 矛盾仍必須 fail closed，缺少 `valid` 保留既有 unverified/未記錄語意，不得從 hash、errors 或其他 quality gate 借用結果。
...
- decision tracking、final audit、structured-output warnings、content-credibility confidence calibration 與 workflow state validation 對 `circuit_breaker._ever_opened` 也沿用 shared `safe_bool`；legacy `"false"` 不得誤降 fresh-data confidence cap 或在 checkpoint 製造虛假的開啟歷史，明確 true 與 `status=open` 仍保留既有修復訊號。
- yfinance snapshot adapter、core-provider success gate 與 snapshot assembly 對 `is_valid` 也沿用 shared `safe_bool`；缺值維持 valid-by-default，legacy `"false"` 不得跳過 invalid snapshot error 或阻止既有 fallback provider，snapshot validity 不得透過字串 truthiness 進入核心資料組裝。
- `watchlist_store_items.normalize_item`、`decision_tracking_store.upsert_item`、watchlist screener route 與 `market_screener.run_daily_market_screener` 對外部 `enabled`／`force`／`success` 也沿用 shared `safe_bool`；legacy `"false"` 不得重新啟用項目、觸發 force scan、把 failed scan 投影成 success，或在 failed scan 時進入 Auto-Screener prune。缺值預設與 SQLite `0/1` row projection 維持既有語意。
- `api_observability_prometheus._metric_bool` 對 queue availability、free-mode 與 observability flags 也沿用 shared `safe_bool`；`available/unavailable` 保留專用相容 token，unknown truthy token 不得透過 raw truthiness 變成 healthy/available gauge。
- `strategy_evaluator._bool_or_none()` 對 `metrics.hit` 的明確 bool、0/1 與 true/false token 也沿用 shared `safe_bool`；legacy `"false"` 不得在 `outcome=hit` 下變成 hit rate，malformed object 仍沿用既有 outcome fallback。
- `watchlist_trigger_store.record_trigger_event()` 對 event `matched` 的寫入與 duplicate update gate 也沿用 shared `safe_bool`；legacy `"false"` 不得把既有 unmatched event promote 成 matched 或觸發錯誤 enqueue，SQLite integer row projection 維持既有語意。
...
- 報告預覽的 quality action 對 `snapshot_integrity.status=invalid` 或 `valid=false` 產生 critical `manual_review`，保留 hash／error detail，與 blocked reading boundary 及後端 repair queue 的 snapshot 優先級對齊；此 projection 不改 snapshot、artifact、index、review、queue 或 rerun state。
- 報告預覽的 snapshot quality action 會移除 generic blocker 並保留同一筆記錄的具體 provider／hash detail，與 reading notice、repair queue 的 evidence precedence 對齊；重複或無具體 detail 時仍保留既有 fallback，不改 persisted state。
...
- `report_quality_repair_queue` 的 repair item pipeline 由共用 `report_pipeline_identity.resolve_report_pipeline_id()` 解析；有效 stored pipeline 優先，placeholder 時回退報告 filename。這個 canonical identity 必須在 repair action 輸出邊界完成，避免操作員看到 v4 報告卻收到 `N/A`／v1 的標籤；它仍是 read-only projection，不改 report、snapshot、artifact、index、review 或 queue state。
- current-quality、quality-audit、freshness、provider-impact 與 daily decision 的 report-facing targets 也在各自 output boundary 使用同一 resolver；有效 stored/index 或 snapshot identity 優先，placeholder 才回退 filename，沒有可解析 filename 時才保留 v1 default。這讓品質、來源維運、重跑、回測與人工 action 對同一份 v4 artifact 保持相同 pipeline label，不改 gate、snapshot、artifact、index、review、queue 或 rerun state。
- `decision_backtest_service` 與 `decision_tracking_store` 在回測結果的 evaluation/operational persistence boundary 使用同一 resolver；`outcome_calibration` 再用 report filename、backtest identity 與 report identity 的 canonical 結果建立 details、by-pipeline 與 quality attribution。legacy `N/A` 不會污染 v4 的歷史命中率分組，也不會因 identity 修正而重算價格、outcome、quality gate 或報告 artifact。
- historical quality audit 的 `_latest_report_filenames` 與 `_indexed_report_version_status` 也以 filename-aware resolver 建立 ticker/pipeline identity；因此 legacy placeholder 不會把 v4 indexed version 變成 `unknown`，current/historical coverage 仍依 valid pipeline 分組，且只改 read-only audit classification。
- daily dashboard 的 `_report_identity_key` 在 `repair_sample_overlap` 比對 sample row 與 quality-audit item 前，也以 filename-aware resolver 收斂 pipeline；同一 v4 artifact 即使一側仍為 `N/A`，也不會被錯算成 sample 外品質缺口。
...
- historical quality audit 另外以 report index 的 latest-per-ticker/pipeline 結果標記 `report_version_status=current|historical|unknown`，並以 `quality_metadata_missing_by_version_status` 呈現缺口分布；有提供時三個 version bucket 必須完整分割 missing 分母，shared audit-scope helper 遇到 malformed 或矛盾時不渲染版本摘要。這是解讀範圍的 read-only evidence，不改歷史 artifact、review ledger、rerun queue 或 report index。
...
- `report_quality_review_store` 與 `report_quality_review_workflow` 共用 `report_pipeline_identity.resolve_report_pipeline_id()`：stored/index pipeline 有效時優先，placeholder 時由 snapshot 或報告檔名補足；review write/read 與 target lookup 必須使用同一 canonical filename/pipeline/revision key。讀取時會把 legacy placeholder pipeline 事件在記憶體中歸一，不回寫歷史 ledger，也不以另一份報告推測模式。
...
- refresh response 的 `decision_freshness` 必須直接以剛保存的 refreshed snapshot 計算，不可再以 `output_dir + storage key` 重讀；這讓 InMemory/remote-like storage 與 filesystem storage 共享同一 freshness truth，避免有效的 `needs_rerun` 被錯誤降級成 `unknown`。
...
- 瀏覽器 evidence helper 對 `quality_metadata_before_refresh`／`quality_metadata_after_refresh` reason code 先做 trim/lowercase projection，再投影 preview、daily target 與 historical target 的來源提示；這只修正 read-only rendering，不回寫 persisted reason code。
- `reporting.data_trust_summary` 對 generated HTML/Markdown 的 reason labels 使用 bounded、stable priority projection：具體 source error/stale/provider SLA 先於 generic freshness/note，HTML/Markdown 分別保留既有四項／八項上限，同優先級不重排；不改 trust status 或 persisted reason codes。
...
- report conformance 的 final-audit step 對缺少或 `not_recorded` 的 `final_audit` 回傳 warning 並保留 `details=not_recorded`；明確 critical 或阻斷狀態優先回傳 blocked，不以空 payload 推定 audit passed。這是產製輸出契約的判定，與歷史/API 的缺 metadata projection 分開。
- report conformance 的 `report_lint` 與 `content_credibility` step 只有 `passed` 可回傳 passed；`warning`、`unknown`、`not_recorded` 或其他未知狀態回 warning，`failed`、`rejected`、`blocked` 或明確 blocker 回 blocked，並以大小寫不敏感的狀態比較避免 malformed payload 繞過品質閘門。
...
- `evidence_exit_gate._path_markers_for_claim()` 對明示 `引用`／`來源`／`source` 的 `market_data.week_52_high_twd`／`market_data.week_52_low_twd` citation，容許 Markdown emphasis 與來源詞的格式間距後映射至 `data.week_52_high`／`data.week_52_low`；仍要求同 claim 數值相等與 explicit source name，不借用 generic `risk_price` 或其他同值欄位。
- `evidence_exit_gate._path_markers_for_claim()` 對 52 週高低點 label delimiter 後的 Markdown emphasis gap 採窄幅容忍，讓 `**52 週高點：** 78.2 TWD` 仍可映射 `data.week_52_high`；仍要求 week high/low、貨幣單位與 reported value 相等，不放寬 source adjacency 或 generic fallback。
- `evidence_exit_gate._path_markers_for_claim()` 對壓力／支撐 claim 的 `目前`、`目前處於`、`目前為`、`目前價位即為` current-context qualifier 才映射對應的 `data.week_52_high`／`data.week_52_low`；若高低點語句後緊接第二個數字，negative guard 會阻止把後方數字或前方壓力值錯綁，canonical mismatch 仍保留 mismatch，target／stop-loss／次要價格不借用。
- `evidence_exit_gate._path_markers_for_claim()` 對 exact `波段天花板` label 也只在同一 claim 明示 `52 週最高價`、`market_data` 與 TWD／元數值時映射 `data.week_52_high`；沒有 week marker 的同名壓力 prose、歷史高點、target 或 stop-loss 不建立 path。
- `evidence_exit_gate._path_markers_for_claim()` 對報告數字後方明示的 `week_52_high_twd`／`week_52_low_twd`、`market_data.week_52_*_twd` 與 `financial JSON` source key，取同一 claim 中 source key 前最近且等於 reported value 的數字，映射到 `data.week_52_high`／`data.week_52_low`；數值不符仍保留該 canonical path 的 `snapshot_value_mismatch`，generic 高低點、次要價格與無專用 source key 的數字不建立 path。
- `evidence_exit_gate._path_markers_for_claim()` 對 `Last 10 days daily trend` 的 `Aug NN` row 沿用既有 institutional daily series binding，只有在 series context 與唯一 snapshot 年份成立時映射 `data.institutional_trading.daily_total_net_buy_last_10[YYYY-MM-DD].net_buy_thousand_shares`；standalone month-day 不猜測，數值不符保留 dated mismatch。
- `evidence_exit_gate._path_markers_for_claim()` 同樣只接受 exact `Recent Trend (Last 10 days)` 作為 legacy institutional daily-series heading；仍要求 series context 與唯一 snapshot 年份，其他 trend prose 不建立 institutional path，數值不符保留 dated mismatch。
- `evidence_exit_gate._path_markers_for_claim()` 也接受 exact `Daily Trend (Last 10 days)` 與 `Last 10 days daily total net buy` heading 變體；`Aug NN` 仍只有在法人 daily-series context 與唯一 snapshot 年份成立時映射到 dated `daily_total_net_buy_last_10` path，避免把 standalone month-day 借成 evidence。
- `evidence_exit_gate._path_markers_for_claim()` 對 exact `Retail` label 只映射 `retail_holders_lt_50_lots_pct`，讓 TDCC `<50 lots` holder distribution 可核驗；不跨到 major-holder、行情或其他同值欄位，數值不符仍保留該 holder path 的 mismatch。
- `evidence_exit_gate._path_markers_for_claim()` 對 `Equity Multiplier`／`權益乘數` 只映射 `equity_multiplier`；`_is_eligible_snapshot_value()` 排除 `equity_multiplier_note` 口徑提示，避免 note 中的同值取代專用 canonical field，數值不符仍保留 `data.equity_multiplier` mismatch。
- `evidence_exit_gate._path_markers_for_claim()` 對歷史 `CAGR (5yr)`、TDCC holder lots 別名、`目前價格`／`當前市價`、中文借券 flow 別名與 `Margin Previous Balance` 只映射各自的 canonical snapshot path；不跨欄位使用同值 candidate。借券償還仍依 claim 單位將 raw shares 轉換，轉換後不一致時保留 `snapshot_value_mismatch`，讓資料品質問題可追蹤。
- `evidence_exit_gate._path_markers_for_claim()` 對 exact `Latest annual growth` 固定映射 `latest_annual_revenue_growth`，不因 `latest_annual_net_income_growth` 同值而跨欄位取證；缺少 revenue canonical value 時保留不可驗證，數值不符仍保留 revenue path mismatch。
- `evidence_exit_gate._path_markers_for_claim()` 對 `近 5 個交易日淨買超 (千股)` 與有五日上下文的 `法人合計買超` 固定映射 `institutional_trading.last_5_trading_days_net_buy_thousand_shares`；`小戶 (持股 < 50 張) 比例` 固定映射 TDCC retail-holder path，`權益負債比率` 固定映射 `debt_to_equity`。standalone `S&P 500` 只有在 percentage claim 明示 `1d` 時映射 SPY `change_1d_pct`，不把指數目標或其他 symbol 借到該 path。
- `evidence_exit_gate._path_markers_for_claim()` 對短 label `Trust` 只有在同一 raw line 明示 `Net Buy (30 days)` 且依序包含 `Foreign`、`Dealer`、`Trust` 時映射 `institutional_trading.net_buy_thousand_shares_by_category.investment_trust`；孤立或缺少完整分類上下文的 `Trust` 不建立 path，避免跨法人分類借值。
- `evidence_exit_gate._is_eligible_snapshot_value()` 將 generic `confidence` path（包含 `confidence_basis.evidence_items`）視為分析 metadata，排除在 factual evidence candidates 外；confidence claim 沒有 dedicated canonical path 時只輸出 `confidence_metadata_not_evidence`，不借用 DCF、target、risk 或其他同值數字。
- `evidence_exit_gate._check_claim()` 對 `情緒評分`、`FOMO Score` 等情緒／過熱分數，在沒有 dedicated canonical score field 時輸出 `analysis_metadata_not_evidence`；`social_sentiment` freshness metadata、`data_trust.score` 與 data-confidence score 不作分數 evidence，也不借用其他同值數字。
- `evidence_exit_gate._path_markers_for_claim()` 對 recommendation-prefix horizon label（例如 `放空；3個月`）只有在 parsed/structured context 存在時映射到對應的 `rerun_context.parsed.recommendation` horizon path；legacy context 缺 source 時仍維持 `legacy_conclusion_without_snapshot_path`，不借用同值 target。
...
- `evidence_exit_gate._FIELD_HINTS` 對 `流動比率`／`Current Ratio` 只映射 `data.current_ratio`，對 `債務權益比`／`Debt to Equity` 只映射 `data.debt_to_equity`；值不一致時保留 `snapshot_value_mismatch`，不借用其他財務或風險欄位。
...
- `evidence_exit_gate._path_markers_for_claim()` 對 `Borrowed Short Sale Today` 建立 `borrowed_short_sale_today` marker，對 `Borrowed Short Return Today`／`Today's borrowed short return`／`Return Today` 與同一 borrowed-short-sale claim 中的 compact `return` 建立 `chip_data.twse_margin_short_sales.borrowed_short_return_today` marker；`k`、`張`、`M` 只在 claim 明示相應單位時分別做千股、張、百萬股 conversion。精確 `vs Sale Today` 仍建立 sale path，兩條 path 不互借。
...
- `report_quality_audit.quality_metadata_missing_by_provenance` 只針對缺 metadata 的 verified rows 分成 `before_refresh`、`after_refresh` 與 `no_refresh_provenance`；有提供時三者必須完整分割並加總回 missing 分母，shared audit-scope helper 遇到 malformed 或算術矛盾時不渲染該分布。`before_refresh` 需由 pre-refresh trace 覆蓋目前缺口，`after_refresh` 是有刷新 attribution 但無此覆蓋，`no_refresh_provenance` 沒有可用分類，三者都不是 gate 結果。明細另保留 `quality_metadata_provenance`、`quality_metadata_refresh_provenance`、`refreshed_from_report`、`snapshot_refreshed_at`，讓人工核對能回到 artifact/freshness 證據。
- `report_quality_audit.quality_metadata_missing_by_rerun_execution` 只統計 verified snapshot 的 quality metadata 缺口，沿用 `quality_metadata_repair_item()` 的 `rerun_execution_status`；有提供時各策略 bucket 必須完整分割並加總回 missing 分母，shared audit-scope helper 遇到 malformed 或算術矛盾時不渲染該分布。沒有足夠 refresh provenance 的 item 進入 `not_evaluated`，不把未知誤報成 unavailable。它是人工排序摘要，不是 rerun enqueue 或 freshness override；`full_rerun_required` 優先於 artifact fallback，避免把可讀的 Markdown 前序段落誤當成可局部重跑的授權。
- `report_quality_audit.quality_metadata_missing_by_rerun_context` 只統計同一批 verified 缺口的上下文證據，分成 `present`、`partial`、`artifact_fallback_available`、`missing` 與 `not_evaluated`；有提供時各 bucket 必須完整分割並加總回 missing 分母，shared audit-scope helper 遇到 malformed 或算術矛盾時不渲染該分布。它與 execution strategy 分開，僅供 read-only 工作準備與人工排序，不改 freshness、review、rerun、repair 或 queue。
- `report_quality_audit.quality_metadata_by_pipeline` 以 `pipeline_id` 分組保留 verified 分母、coverage basis、缺 gate counts、execution strategy 與 context readiness；各 entry 的 `quality_metadata_missing_reports` 必須完整分割並加總回 top-level 缺口，每個 entry 的 context map 也必須為合法有限非負整數且加總回自身 missing count。shared audit-scope helper 遇到 malformed、缺 entry 或任一層算術矛盾時不渲染「模式缺口」或「模式上下文」，避免把部分分布當成全量結論。一次 historical response 可完成模式優先級判斷；有效 partition 時 history/watchlist 以「模式上下文」呈現準備差異，詳細明細仍用 `q`/`pipeline` targeted audit，且不做 side effect。
