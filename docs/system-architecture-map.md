...
- `content_credibility_inputs.first_price()` 先移除明確日曆日期與週期數字（包含 `8/18`、完整日期，以及 `1-2週`、`1至2週`、`1 to 2 weeks` 這類期間範圍），再呼叫既有價格 parser，避免交易計畫的日期／週期 token 污染目標價或停損價；同一 input boundary 另保留 `price_candidates()`，讓 mode-D 對多個非區間情境價格產生 read-only `ambiguous_trade_setup_price_inputs` warning 與候選值。`_PRICE_RANGE_PATTERN` 允許 `NT$`、`$`、`TWD`、`元` 出現在兩端價格附近，因此 `121.0 TWD 至 130.5 TWD` 這類明確區間維持合法；Neutral 政策、snapshot、index、artifact、gate persistence 或 queue 不因此改寫。
