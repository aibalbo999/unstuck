...
- `report_quality_audit.quality_metadata_by_pipeline` 以 `pipeline_id` 分組保留 verified 分母、coverage basis、缺 gate counts、execution strategy 與 context readiness；各 entry 的 `quality_metadata_missing_reports` 必須完整分割並加總回 top-level 缺口，shared audit-scope helper 遇到 malformed、缺 entry 或算術矛盾時不渲染「模式缺口」或「模式上下文」，避免把部分分布當成全量結論。一次 historical response 可完成模式優先級判斷；有效 partition 時 history/watchlist 以「模式上下文」呈現準備差異，詳細明細仍用 `q`/`pipeline` targeted audit，且不做 side effect。
