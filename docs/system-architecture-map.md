...
- history workspace 只有在 `顯示舊版報告` 開啟時呼叫 historical audit，沿用目前的 `q`/pipeline 篩選；摘要與最多五個 report target 由獨立 `history_quality_audit` module 渲染，CTA 只委派既有 `openReport()`，不把 read-only audit 轉成 daily action。
