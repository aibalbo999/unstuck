...
- `/api/watchlist/report-quality-audit/historical` 是獨立的 read-only historical scope，使用 `include_versions=True` 掃描所有 indexed versions，並以 `scope=all_historical_indexed_reports`、`selection_basis=all_indexed_versions` 明示範圍；它不進 daily decision queue，也不做 artifact/index/rerun side effect。
