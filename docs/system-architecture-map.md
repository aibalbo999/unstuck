...
- historical quality audit renderer 對 audit、gap、provenance、rerun、review、version、artifact、snapshot、pipeline 與 bounded item counts 採有限非負整數邊界，且核心 gap count 必須不大於 audited scope；若有 verified snapshot scope，還必須滿足 `missing <= verified <= audited`。fractional/malformed 或彼此矛盾的值不渲染，核心範圍不完整時顯示「資料需確認」，不以 `floor` 產生假報告數。
