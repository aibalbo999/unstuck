...
- `evidence_exit_gate._path_markers_for_claim()` 對 30 天法人交易的 exact `Foreign`／`外資` 與 `Investment Trust`／`投信` label 建立各自的 `net_buy_thousand_shares_by_category.foreign`／`investment_trust` marker；generic `Total` 不建立總額 path，避免分類 claim 與總額欄位跨語意碰撞。
